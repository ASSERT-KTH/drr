...
public class Percentile extends org.apache.commons.math.stat.descriptive.AbstractUnivariateStatistic implements java.io.Serializable {
	...
	public Percentile(org.apache.commons.math.stat.descriptive.rank.Percentile original) {
		org.apache.commons.math.stat.descriptive.rank.Percentile.copy(original, org.apache.commons.math.stat.descriptive.rank.Percentile.this);
	}
	...
	public void setData(final double[] values) {
		if (values == null) {
			...
			cachedPivots = new int[(1 << (org.apache.commons.math.stat.descriptive.rank.Percentile.MAX_CACHED_LEVELS)) - 1];
	...
	public double evaluate(final double[] values, final int begin, final int length, final double p) {
		...
		if ((p > 100) || (p <= 0)) {
			throw new org.apache.commons.math.exception.OutOfRangeException(org.apache.commons.math.exception.util.LocalizedFormats.OUT_OF_BOUNDS_QUANTILE_VALUE, p, 0, 100);
		}
		if (length == 0) {
		...
		if (values == (getDataRef())) {
			...
			pivotsHeap = new int[(1 << (org.apache.commons.math.stat.descriptive.rank.Percentile.MAX_CACHED_LEVELS)) - 1];
	...
	private double select(final double[] work, final int[] pivotsHeap, final int k) {
		...
		int end = work.length;
		int node = 0;
		while ((end - begin) > (org.apache.commons.math.stat.descriptive.rank.Percentile.MIN_SELECT_SIZE)) {
			final int pivot;
	...
	public org.apache.commons.math.stat.descriptive.rank.Percentile copy() {
		...
		org.apache.commons.math.stat.descriptive.rank.Percentile.copy(org.apache.commons.math.stat.descriptive.rank.Percentile.this, result);
		return result;
	}
