...
public class PearsonsCorrelation {
	...
	public org.apache.commons.math.linear.RealMatrix getCorrelationStandardErrors() {
		...
		for (int i = 0; i < nVars; i++) {
			for (int j = 0; j < nVars; j++) {
				...
				out[i][j] = org.apache.commons.math.util.FastMath.sqrt(((1 - (r * r)) / ((nObs) - 2)));
			}
		}
	...
	public org.apache.commons.math.linear.RealMatrix getCorrelationPValues() throws org.apache.commons.math.MathException {
		org.apache.commons.math.distribution.TDistribution tDistribution = new org.apache.commons.math.distribution.TDistribution(((nObs) - 2));
		int nVars = correlationMatrix.getColumnDimension();
		double[][] out = new double[nVars][nVars];
		for (int i = 0; i < nVars; i++) {
			for (int j = 0; j < nVars; j++) {
				if (i == j) {
					...
					double r = correlationMatrix.getEntry(i, j);
					double t = org.apache.commons.math.util.FastMath.abs((r * (org.apache.commons.math.util.FastMath.sqrt((((nObs) - 2) / (1 - (r * r)))))));
	...
	public org.apache.commons.math.linear.RealMatrix covarianceToCorrelation(org.apache.commons.math.linear.RealMatrix covarianceMatrix) {
		...
		for (int i = 0; i < nVars; i++) {
			...
			outMatrix.setEntry(i, i, 1.0);
			for (int j = 0; j < i; j++) {
				double entry = (covarianceMatrix.getEntry(i, j)) / (sigma * (org.apache.commons.math.util.FastMath.sqrt(covarianceMatrix.getEntry(j, j))));
