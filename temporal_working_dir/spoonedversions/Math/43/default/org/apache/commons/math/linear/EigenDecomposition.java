...
public class EigenDecomposition {
	...
	private boolean isSymmetric(final org.apache.commons.math.linear.RealMatrix matrix, boolean raiseException) {
		...
		final double eps = ((10 * rows) * columns) * (org.apache.commons.math.util.Precision.EPSILON);
	...
	private void findEigenVectors(double[][] householderMatrix) {
		...
		if (maxAbsoluteValue != 0) {
			for (int i = 0; i < n; i++) {
				if ((org.apache.commons.math.util.FastMath.abs(realEigenvalues[i])) <= ((org.apache.commons.math.util.Precision.EPSILON) * maxAbsoluteValue)) {
					realEigenvalues[i] = 0;
				}
				if ((org.apache.commons.math.util.FastMath.abs(e[i])) <= ((org.apache.commons.math.util.Precision.EPSILON) * maxAbsoluteValue)) {
					e[i] = 0;
				}
			}
		}
