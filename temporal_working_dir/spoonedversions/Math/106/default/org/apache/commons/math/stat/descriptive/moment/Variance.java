			...
			if (length == 1) {
				...
				if (length > 1) {
					...
					for (int i = begin; i < (begin + length); i++) {
						accum += java.lang.Math.pow(((values[i]) - mean), 2.0);
						accum2 += (values[i]) - mean;
					}
					if (isBiasCorrected) {
						var = (accum - ((java.lang.Math.pow(accum2, 2)) / ((double) (length)))) / ((double) (length - 1));
					}else {
						var = (accum - ((java.lang.Math.pow(accum2, 2)) / ((double) (length)))) / ((double) (length));
					}
				}
