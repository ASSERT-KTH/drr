...
public class CMAESOptimizer extends org.apache.commons.math.optimization.direct.BaseAbstractMultivariateOptimizer<org.apache.commons.math.analysis.MultivariateFunction> implements org.apache.commons.math.optimization.MultivariateOptimizer {
	...
	public CMAESOptimizer(int lambda, double[] inputSigma, double[][] boundaries, int maxIterations, double stopFitness, boolean isActiveCMA, int diagonalOnly, int checkFeasableCount, org.apache.commons.math.random.RandomGenerator random, boolean generateStatistics) {
		this(lambda, inputSigma, boundaries, maxIterations, stopFitness, isActiveCMA, diagonalOnly, checkFeasableCount, random, generateStatistics, new org.apache.commons.math.optimization.SimpleValueChecker());
	}

	public CMAESOptimizer(int lambda, double[] inputSigma, double[][] boundaries, int maxIterations, double stopFitness, boolean isActiveCMA, int diagonalOnly, int checkFeasableCount, org.apache.commons.math.random.RandomGenerator random, boolean generateStatistics, org.apache.commons.math.optimization.ConvergenceChecker<org.apache.commons.math.optimization.PointValuePair> checker) {
		super(checker);
	...
	protected org.apache.commons.math.optimization.PointValuePair doOptimize() {
		...
		org.apache.commons.math.optimization.PointValuePair optimum = new org.apache.commons.math.optimization.PointValuePair(getStartPoint(), (isMinimize ? bestValue : -bestValue));
		org.apache.commons.math.optimization.PointValuePair lastResult = null;
		generationLoop : for (iterations = 1; (iterations) <= (maxIterations); (iterations)++) {
			...
			org.apache.commons.math.linear.RealMatrix bestArx = org.apache.commons.math.optimization.direct.CMAESOptimizer.selectColumns(arx, org.apache.commons.math.util.MathArrays.copyOf(arindex, mu));
			xmean = bestArx.multiply(weights);
			org.apache.commons.math.linear.RealMatrix bestArz = org.apache.commons.math.optimization.direct.CMAESOptimizer.selectColumns(arz, org.apache.commons.math.util.MathArrays.copyOf(arindex, mu));
			...
			if (bestValue > bestFitness) {
				...
				lastResult = optimum;
				optimum = new org.apache.commons.math.optimization.PointValuePair(fitfun.decode(bestArx.getColumn(0)), (isMinimize ? bestFitness : -bestFitness));
			...
			if ((getConvergenceChecker()) != null) {
				org.apache.commons.math.optimization.PointValuePair current = new org.apache.commons.math.optimization.PointValuePair(bestArx.getColumn(0), (isMinimize ? bestFitness : -bestFitness));
			...
			for (int i = 0; i < (init.length); i++) {
				...
				if ((boundaries) != null) {
					if ((inputSigma[i]) > ((boundaries[1][i]) - (boundaries[0][i]))) {
						throw new org.apache.commons.math.exception.OutOfRangeException(inputSigma[i], 0, ((boundaries[1][i]) - (boundaries[0][i])));
					}
				}
			}
		}
	...
	private void updateCovariance(boolean hsig, final org.apache.commons.math.linear.RealMatrix bestArx, final org.apache.commons.math.linear.RealMatrix arz, final int[] arindex, final org.apache.commons.math.linear.RealMatrix xold) {
		...
		if (((ccov1) + (ccovmu)) > 0) {
			...
			if (isActiveCMA) {
				...
				org.apache.commons.math.linear.RealMatrix arzneg = org.apache.commons.math.optimization.direct.CMAESOptimizer.selectColumns(arz, org.apache.commons.math.util.MathArrays.copyOf(arReverseIndex, mu));
	...
	private void updateBD(double negccov) {
		if (((((ccov1) + (ccovmu)) + negccov) > 0) && ((((((iterations) % 1.0) / (((ccov1) + (ccovmu)) + negccov)) / (dimension)) / 10.0) < 1.0)) {
			...
			org.apache.commons.math.linear.EigenDecomposition eig = new org.apache.commons.math.linear.EigenDecomposition(C, 1.0);
			B = eig.getV();
			D = eig.getD();
