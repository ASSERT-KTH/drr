...
public class BracketingNthOrderBrentSolver extends org.apache.commons.math.analysis.solvers.AbstractUnivariateSolver implements org.apache.commons.math.analysis.solvers.BracketedUnivariateSolver<org.apache.commons.math.analysis.UnivariateFunction> {
	...
	protected double doSolve() {
		...
		if (org.apache.commons.math.util.Precision.equals(y[1], 0.0, 1)) {
			return x[1];
		}
		y[0] = computeObjectiveValue(x[0]);
		if (org.apache.commons.math.util.Precision.equals(y[0], 0.0, 1)) {
			return x[0];
		}
		int nbPoints;
		int signChangeIndex;
		...
		while (true) {
			...
			if (agingA >= (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.MAXIMAL_AGING)) {
				final int p = agingA - (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.MAXIMAL_AGING);
				final double weightA = (1 << p) - 1;
				final double weightB = p + 1;
				targetY = ((weightA * yA) - ((weightB * (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.REDUCTION_FACTOR)) * yB)) / (weightA + weightB);
			}else
				if (agingB >= (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.MAXIMAL_AGING)) {
					final int p = agingB - (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.MAXIMAL_AGING);
					final double weightA = p + 1;
					final double weightB = (1 << p) - 1;
					targetY = ((weightB * yB) - ((weightA * (org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.REDUCTION_FACTOR)) * yA)) / (weightA + weightB);
				}else {
					targetY = 0;
				}
	...
	public double solve(int maxEval, org.apache.commons.math.analysis.UnivariateFunction f, double min, double max, double startValue, org.apache.commons.math.analysis.solvers.AllowedSolution allowedSolution) {
		org.apache.commons.math.analysis.solvers.BracketingNthOrderBrentSolver.this.allowed = allowedSolution;
