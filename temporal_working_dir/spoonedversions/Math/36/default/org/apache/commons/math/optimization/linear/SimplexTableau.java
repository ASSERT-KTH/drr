...
class SimplexTableau implements java.io.Serializable {
	...
	SimplexTableau(final org.apache.commons.math.optimization.linear.LinearObjectiveFunction f, final java.util.Collection<org.apache.commons.math.optimization.linear.LinearConstraint> constraints, final org.apache.commons.math.optimization.GoalType goalType, final boolean restrictToNonNegative, final double epsilon) {
		this(f, constraints, goalType, restrictToNonNegative, epsilon, org.apache.commons.math.optimization.linear.SimplexTableau.DEFAULT_ULPS);
	}
	...
	SimplexTableau(final org.apache.commons.math.optimization.linear.LinearObjectiveFunction f, final java.util.Collection<org.apache.commons.math.optimization.linear.LinearConstraint> constraints, final org.apache.commons.math.optimization.GoalType goalType, final boolean restrictToNonNegative, final double epsilon, final int maxUlps) {
		...
		org.apache.commons.math.optimization.linear.SimplexTableau.this.tableau = createTableau((goalType == (org.apache.commons.math.optimization.GoalType.MAXIMIZE)));
	...
	protected org.apache.commons.math.linear.RealMatrix createTableau(final boolean maximize) {
		...
		if (!(restrictToNonNegative)) {
			matrix.setEntry(zIndex, ((getSlackVariableOffset()) - 1), org.apache.commons.math.optimization.linear.SimplexTableau.getInvertedCoefficientSum(objectiveCoefficients));
		}
		int slackVar = 0;
		int artificialVar = 0;
		for (int i = 0; i < (constraints.size()); i++) {
			...
			if (!(restrictToNonNegative)) {
				matrix.setEntry(row, ((getSlackVariableOffset()) - 1), org.apache.commons.math.optimization.linear.SimplexTableau.getInvertedCoefficientSum(constraint.getCoefficients()));
			}
	...
	protected static double getInvertedCoefficientSum(final org.apache.commons.math.linear.RealVector coefficients) {
		double sum = 0;
	...
	protected java.lang.Integer getBasicRow(final int col) {
		...
		for (int i = 0; i < (getHeight()); i++) {
			...
			if ((org.apache.commons.math.util.Precision.equals(entry, 1.0, maxUlps)) && (row == null)) {
				row = i;
			}else
				if (!(org.apache.commons.math.util.Precision.equals(entry, 0.0, maxUlps))) {
					return null;
				}
	...
	protected void dropPhase1Objective() {
		...
		for (int i = getNumObjectiveFunctions(); i < (getArtificialVariableOffset()); i++) {
			...
			if ((org.apache.commons.math.util.Precision.compareTo(entry, 0.0, maxUlps)) > 0) {
				columnsToDrop.add(i);
			}
		}
		for (int i = 0; i < (getNumArtificialVariables()); i++) {
		...
		org.apache.commons.math.optimization.linear.SimplexTableau.this.tableau = new org.apache.commons.math.linear.Array2DRowRealMatrix(matrix);
	...
	boolean isOptimal() {
		for (int i = getNumObjectiveFunctions(); i < ((getWidth()) - 1); i++) {
			final double entry = tableau.getEntry(0, i);
			if ((org.apache.commons.math.util.Precision.compareTo(entry, 0.0, epsilon)) < 0) {
				return false;
			}
		}
		return true;
	...
	protected org.apache.commons.math.optimization.PointValuePair getSolution() {
		...
		return new org.apache.commons.math.optimization.PointValuePair(coefficients, f.getValue(coefficients));
	}
