...
public class MultivariateFunctionPenaltyAdapter implements org.apache.commons.math3.analysis.MultivariateFunction {
	...
	public MultivariateFunctionPenaltyAdapter(final org.apache.commons.math3.analysis.MultivariateFunction bounded, final double[] lower, final double[] upper, final double offset, final double[] scale) {
		org.apache.commons.math3.util.MathUtils.checkNotNull(lower);
		org.apache.commons.math3.util.MathUtils.checkNotNull(upper);
		org.apache.commons.math3.util.MathUtils.checkNotNull(scale);
		...
		if ((lower.length) != (scale.length)) {
			throw new org.apache.commons.math3.exception.DimensionMismatchException(lower.length, scale.length);
		}
		...
		for (int i = 0; i < (scale.length); ++i) {
			if (((point[i]) < (lower[i])) || ((point[i]) > (upper[i]))) {
				...
				for (int j = i; j < (scale.length); ++j) {
					...
					sum += org.apache.commons.math3.util.FastMath.sqrt(overshoot);
				}
				return (offset) + sum;
			}
		}
