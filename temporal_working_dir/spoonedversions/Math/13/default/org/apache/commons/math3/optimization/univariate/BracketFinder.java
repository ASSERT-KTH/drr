...
public class BracketFinder {
	...
	public void search(org.apache.commons.math3.analysis.UnivariateFunction func, org.apache.commons.math3.optimization.GoalType goal, double xA, double xB) {
		evaluations.resetCount();
		final boolean isMinim = goal == (org.apache.commons.math3.optimization.GoalType.MINIMIZE);
		...
		double xC = xB + ((org.apache.commons.math3.optimization.univariate.BracketFinder.GOLD) * (xB - xA));
		double fC = eval(func, xC);
		while (isMinim ? fC < fB : fC > fB) {
			double tmp1 = (xB - xA) * (fB - fC);
			double tmp2 = (xB - xC) * (fB - fA);
			double val = tmp2 - tmp1;
			double denom = (java.lang.Math.abs(val)) < (org.apache.commons.math3.optimization.univariate.BracketFinder.EPS_MIN) ? 2 * (org.apache.commons.math3.optimization.univariate.BracketFinder.EPS_MIN) : 2 * val;
			...
			if (((w - xC) * (xB - w)) > 0) {
				...
				w = xC + ((org.apache.commons.math3.optimization.univariate.BracketFinder.GOLD) * (xC - xB));
				fW = eval(func, w);
			}else
				if (((w - wLim) * (wLim - xC)) >= 0) {
					w = wLim;
