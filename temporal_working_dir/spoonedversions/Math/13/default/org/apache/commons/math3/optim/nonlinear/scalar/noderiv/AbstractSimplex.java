...
public abstract class AbstractSimplex implements org.apache.commons.math3.optim.OptimizationData {
	...
	protected AbstractSimplex(int n, double sideLength) {
		this(org.apache.commons.math3.optim.nonlinear.scalar.noderiv.AbstractSimplex.createHypercubeSteps(n, sideLength));
	}
	...
	public abstract void iterate(final org.apache.commons.math3.analysis.MultivariateFunction evaluationFunction, final java.util.Comparator<org.apache.commons.math3.optim.PointValuePair> comparator);

	public void build(final double[] startPoint) {
		if ((dimension) != (startPoint.length)) {
			throw new org.apache.commons.math3.exception.DimensionMismatchException(dimension, startPoint.length);
		}
		simplex = new org.apache.commons.math3.optim.PointValuePair[(dimension) + 1];
		simplex[0] = new org.apache.commons.math3.optim.PointValuePair(startPoint, java.lang.Double.NaN);
		for (int i = 0; i < (dimension); i++) {
			...
			simplex[(i + 1)] = new org.apache.commons.math3.optim.PointValuePair(vertexI, java.lang.Double.NaN);
		}
	}

	public void evaluate(final org.apache.commons.math3.analysis.MultivariateFunction evaluationFunction, final java.util.Comparator<org.apache.commons.math3.optim.PointValuePair> comparator) {
		for (int i = 0; i < (simplex.length); i++) {
			final org.apache.commons.math3.optim.PointValuePair vertex = simplex[i];
			final double[] point = vertex.getPointRef();
			if (java.lang.Double.isNaN(vertex.getValue())) {
				simplex[i] = new org.apache.commons.math3.optim.PointValuePair(point, evaluationFunction.value(point), false);
			}
		}
		java.util.Arrays.sort(simplex, comparator);
	}

	protected void replaceWorstPoint(org.apache.commons.math3.optim.PointValuePair pointValuePair, final java.util.Comparator<org.apache.commons.math3.optim.PointValuePair> comparator) {
		for (int i = 0; i < (dimension); i++) {
			if ((comparator.compare(simplex[i], pointValuePair)) > 0) {
				org.apache.commons.math3.optim.PointValuePair tmp = simplex[i];
	...
	public org.apache.commons.math3.optim.PointValuePair[] getPoints() {
		final org.apache.commons.math3.optim.PointValuePair[] copy = new org.apache.commons.math3.optim.PointValuePair[simplex.length];
		java.lang.System.arraycopy(simplex, 0, copy, 0, simplex.length);
		return copy;
	}

	public org.apache.commons.math3.optim.PointValuePair getPoint(int index) {
	...
	protected void setPoint(int index, org.apache.commons.math3.optim.PointValuePair point) {
		if ((index < 0) || (index >= (simplex.length))) {
			throw new org.apache.commons.math3.exception.OutOfRangeException(index, 0, ((simplex.length) - 1));
		}
		simplex[index] = point;
	...
	protected void setPoints(org.apache.commons.math3.optim.PointValuePair[] points) {
