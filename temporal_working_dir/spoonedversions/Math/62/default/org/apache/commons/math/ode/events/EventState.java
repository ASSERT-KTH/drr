...
public class EventState {
	...
	public boolean evaluateStep(final org.apache.commons.math.ode.sampling.StepInterpolator interpolator) throws org.apache.commons.math.ConvergenceException, org.apache.commons.math.ode.DerivativeException, org.apache.commons.math.ode.events.EventException {
		try {
			...
			final int n = org.apache.commons.math.util.FastMath.max(1, ((int) (org.apache.commons.math.util.FastMath.ceil(((org.apache.commons.math.util.FastMath.abs((t1 - (t0)))) / (maxCheckInterval))))));
			...
			for (int i = 0; i < n; ++i) {
				...
				if ((g0Positive) ^ (gb >= 0)) {
					...
					final org.apache.commons.math.analysis.solvers.BrentSolver solver = new org.apache.commons.math.analysis.solvers.BrentSolver(maxIterationCount, convergence);
					final double root = ta <= tb ? solver.solve(f, ta, tb) : solver.solve(f, tb, ta);
					if (((org.apache.commons.math.util.FastMath.abs((root - ta))) <= (convergence)) && ((org.apache.commons.math.util.FastMath.abs((root - (previousEventTime)))) <= (convergence))) {
						ta = tb;
						ga = gb;
					}else
						if ((java.lang.Double.isNaN(previousEventTime)) || ((org.apache.commons.math.util.FastMath.abs(((previousEventTime) - root))) > (convergence))) {
							...
							if ((pendingEvent) && ((org.apache.commons.math.util.FastMath.abs((t1 - (pendingEventTime)))) <= (convergence))) {
								return false;
							}
							pendingEvent = true;
							return true;
