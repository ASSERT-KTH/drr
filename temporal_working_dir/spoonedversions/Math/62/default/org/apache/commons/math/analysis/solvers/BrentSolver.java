...
public class BrentSolver extends org.apache.commons.math.analysis.solvers.UnivariateRealSolverImpl {
	...
	public BrentSolver(org.apache.commons.math.analysis.UnivariateRealFunction f) {
		super(f, org.apache.commons.math.analysis.solvers.BrentSolver.DEFAULT_MAXIMUM_ITERATIONS, org.apache.commons.math.analysis.solvers.BrentSolver.DEFAULT_ABSOLUTE_ACCURACY);
	}

	public BrentSolver() {
		super(org.apache.commons.math.analysis.solvers.BrentSolver.DEFAULT_MAXIMUM_ITERATIONS, org.apache.commons.math.analysis.solvers.BrentSolver.DEFAULT_ABSOLUTE_ACCURACY);
	}

	public BrentSolver(double absoluteAccuracy) {
		super(org.apache.commons.math.analysis.solvers.BrentSolver.DEFAULT_MAXIMUM_ITERATIONS, absoluteAccuracy);
	}
	...
	public double solve(final org.apache.commons.math.analysis.UnivariateRealFunction f, final double min, final double max, final double initial) throws org.apache.commons.math.FunctionEvaluationException, org.apache.commons.math.MaxIterationsExceededException {
		...
		if ((initial < min) || (initial > max)) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.INVALID_INTERVAL_INITIAL_VALUE_PARAMETERS, min, initial, max);
		}
		...
		throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.SAME_SIGN_AT_ENDPOINTS, min, max, yMin, yMax);
	}
	...
	private double solve(final org.apache.commons.math.analysis.UnivariateRealFunction f, double x0, double y0, double x1, double y1, double x2, double y2) throws org.apache.commons.math.FunctionEvaluationException, org.apache.commons.math.MaxIterationsExceededException {
		...
		while (i < (maximalIterationCount)) {
			...
			double dx = x2 - x1;
			double tolerance = org.apache.commons.math.util.FastMath.max(((relativeAccuracy) * (org.apache.commons.math.util.FastMath.abs(x1))), absoluteAccuracy);
			if ((org.apache.commons.math.util.FastMath.abs(dx)) <= tolerance) {
				setResult(x1, i);
				return result;
			}
			if (((org.apache.commons.math.util.FastMath.abs(oldDelta)) < tolerance) || ((org.apache.commons.math.util.FastMath.abs(y0)) <= (org.apache.commons.math.util.FastMath.abs(y1)))) {
				delta = 0.5 * dx;
				oldDelta = delta;
			}else {
				double r3 = y1 / y0;
				...
				if (((2.0 * p) >= (((1.5 * dx) * p1) - (org.apache.commons.math.util.FastMath.abs((tolerance * p1))))) || (p >= (org.apache.commons.math.util.FastMath.abs(((0.5 * oldDelta) * p1))))) {
					delta = 0.5 * dx;
					oldDelta = delta;
				}else {
					oldDelta = delta;
			...
			x0 = x1;
			y0 = y1;
			if ((org.apache.commons.math.util.FastMath.abs(delta)) > tolerance) {
				x1 = x1 + delta;
			}else
				if (dx > 0.0) {
					x1 = x1 + (0.5 * tolerance);
