...
class BiDiagonalTransformer {
	...
	public BiDiagonalTransformer(org.apache.commons.math.linear.RealMatrix matrix) {
		...
		final int p = org.apache.commons.math.util.FastMath.min(m, n);
	...
	public org.apache.commons.math.linear.RealMatrix getU() {
		if ((cachedU) == null) {
			...
			cachedU = org.apache.commons.math.linear.MatrixUtils.createRealMatrix(ua);
		}
		return cachedU;
	}
	...
	public org.apache.commons.math.linear.RealMatrix getB() {
		if ((cachedB) == null) {
			...
			cachedB = org.apache.commons.math.linear.MatrixUtils.createRealMatrix(ba);
		}
		return cachedB;
	}
	...
	public org.apache.commons.math.linear.RealMatrix getV() {
		if ((cachedV) == null) {
			...
			cachedV = org.apache.commons.math.linear.MatrixUtils.createRealMatrix(va);
		}
		return cachedV;
	}
	...
	private void transformToUpperBiDiagonal() {
		...
		for (int k = 0; k < n; k++) {
			...
			final double[] hK = householderVectors[k];
			final double a = (hK[k]) > 0 ? -(org.apache.commons.math.util.FastMath.sqrt(xNormSqr)) : org.apache.commons.math.util.FastMath.sqrt(xNormSqr);
			main[k] = a;
			if (a != 0.0) {
				hK[k] -= a;
				for (int j = k + 1; j < n; ++j) {
			...
			if (k < (n - 1)) {
				...
				final double b = (hK[(k + 1)]) > 0 ? -(org.apache.commons.math.util.FastMath.sqrt(xNormSqr)) : org.apache.commons.math.util.FastMath.sqrt(xNormSqr);
				secondary[k] = b;
				if (b != 0.0) {
					hK[(k + 1)] -= b;
					for (int i = k + 1; i < m; ++i) {
	...
	private void transformToLowerBiDiagonal() {
		...
		for (int k = 0; k < m; k++) {
			...
			if (k < (m - 1)) {
				...
				final double b = (hKp1[k]) > 0 ? -(org.apache.commons.math.util.FastMath.sqrt(xNormSqr)) : org.apache.commons.math.util.FastMath.sqrt(xNormSqr);
				secondary[k] = b;
				if (b != 0.0) {
					hKp1[k] -= b;
					for (int j = k + 1; j < n; ++j) {
