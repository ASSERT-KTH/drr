...
public class ContinuousOutputModel implements java.io.Serializable , org.apache.commons.math.ode.sampling.StepHandler {
	...
	public void append(final org.apache.commons.math.ode.ContinuousOutputModel model) throws org.apache.commons.math.exception.MathIllegalArgumentException {
		...
		if ((steps.size()) == 0) {
			...
			if ((getInterpolatedState().length) != (model.getInterpolatedState().length)) {
				throw new org.apache.commons.math.exception.DimensionMismatchException(model.getInterpolatedState().length, getInterpolatedState().length);
			}
			if ((forward) ^ (model.forward)) {
				throw new org.apache.commons.math.exception.MathIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.PROPAGATION_DIRECTION_MISMATCH);
			}
			...
			if ((org.apache.commons.math.util.FastMath.abs(gap)) > (0.001 * (org.apache.commons.math.util.FastMath.abs(step)))) {
				throw new org.apache.commons.math.exception.MathIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.HOLE_BETWEEN_MODELS_TIME_RANGES, org.apache.commons.math.util.FastMath.abs(gap));
			}
		}
	...
	public void setInterpolatedTime(final double time) {
		...
		while ((iMax - iMin) > 5) {
			...
			if (((org.apache.commons.math.util.FastMath.abs((tMed - tMin))) < 1.0E-6) || ((org.apache.commons.math.util.FastMath.abs((tMax - tMed))) < 1.0E-6)) {
				index = iMed;
			}else {
				...
				index = ((int) (org.apache.commons.math.util.FastMath.rint(iLagrange)));
			}
			final int low = org.apache.commons.math.util.FastMath.max((iMin + 1), (((9 * iMin) + iMax) / 10));
			final int high = org.apache.commons.math.util.FastMath.min((iMax - 1), ((iMin + (9 * iMax)) / 10));
			if ((index) < low) {
				index = low;
			}else
				if ((index) > high) {
