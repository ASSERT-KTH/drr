...
public class QRDecomposition {
	...
	public QRDecomposition(org.apache.commons.math3.linear.RealMatrix matrix) {
		this(matrix, 0.0);
	}

	public QRDecomposition(org.apache.commons.math3.linear.RealMatrix matrix, double threshold) {
		this.threshold = threshold;
		final int m = matrix.getRowDimension();
		final int n = matrix.getColumnDimension();
		qrt = matrix.transpose().getData();
		rDiag = new double[org.apache.commons.math3.util.FastMath.min(m, n)];
		cachedQ = null;
		cachedQT = null;
		cachedR = null;
		cachedH = null;
		for (int minor = 0; minor < (org.apache.commons.math3.util.FastMath.min(m, n)); minor++) {
			...
			final double a = (qrtMinor[minor]) > 0 ? -(org.apache.commons.math3.util.FastMath.sqrt(xNormSqr)) : org.apache.commons.math3.util.FastMath.sqrt(xNormSqr);
			rDiag[minor] = a;
			if (a != 0.0) {
				qrtMinor[minor] -= a;
	...
	public org.apache.commons.math3.linear.RealMatrix getR() {
		if ((cachedR) == null) {
			...
			for (int row = (org.apache.commons.math3.util.FastMath.min(m, n)) - 1; row >= 0; row--) {
				ra[row][row] = rDiag[row];
				for (int col = row + 1; col < n; col++) {
					ra[row][col] = qrt[col][row];
				}
			}
			cachedR = org.apache.commons.math3.linear.MatrixUtils.createRealMatrix(ra);
		}
		return cachedR;
	}
	...
	public org.apache.commons.math3.linear.RealMatrix getQT() {
		if ((cachedQT) == null) {
			...
			for (int minor = m - 1; minor >= (org.apache.commons.math3.util.FastMath.min(m, n)); minor--) {
				qta[minor][minor] = 1.0;
			}
			for (int minor = (org.apache.commons.math3.util.FastMath.min(m, n)) - 1; minor >= 0; minor--) {
				final double[] qrtMinor = qrt[minor];
				qta[minor][minor] = 1.0;
	...
	public org.apache.commons.math3.linear.RealMatrix getH() {
		if ((cachedH) == null) {
			...
			for (int i = 0; i < m; ++i) {
				for (int j = 0; j < (org.apache.commons.math3.util.FastMath.min((i + 1), n)); ++j) {
					ha[i][j] = (qrt[j][i]) / (-(rDiag[j]));
				}
			}
			cachedH = org.apache.commons.math3.linear.MatrixUtils.createRealMatrix(ha);
		}
		return cachedH;
	}

	public org.apache.commons.math3.linear.DecompositionSolver getSolver() {
		return new org.apache.commons.math3.linear.QRDecomposition.Solver(qrt, rDiag, threshold);
	}
	...
	private static class Solver implements org.apache.commons.math3.linear.DecompositionSolver {
		...
		public boolean isNonSingular() {
			for (double diag : rDiag) {
				if ((org.apache.commons.math3.util.FastMath.abs(diag)) <= (threshold)) {
					return false;
				}
			}
			return true;
		...
		public org.apache.commons.math3.linear.RealVector solve(org.apache.commons.math3.linear.RealVector b) {
			...
			return new org.apache.commons.math3.linear.ArrayRealVector(x, false);
		}
		...
		public org.apache.commons.math3.linear.RealMatrix solve(org.apache.commons.math3.linear.RealMatrix b) {
			...
			final int blockSize = org.apache.commons.math3.linear.BlockRealMatrix.BLOCK_SIZE;
			final int cBlocks = ((columns + blockSize) - 1) / blockSize;
			final double[][] xBlocks = org.apache.commons.math3.linear.BlockRealMatrix.createBlocksLayout(n, columns);
			final double[][] y = new double[b.getRowDimension()][blockSize];
			final double[] alpha = new double[blockSize];
			for (int kBlock = 0; kBlock < cBlocks; ++kBlock) {
				final int kStart = kBlock * blockSize;
				final int kEnd = org.apache.commons.math3.util.FastMath.min((kStart + blockSize), columns);
			...
			return new org.apache.commons.math3.linear.BlockRealMatrix(n, columns, xBlocks, false);
		}

		public org.apache.commons.math3.linear.RealMatrix getInverse() {
			return solve(org.apache.commons.math3.linear.MatrixUtils.createRealIdentityMatrix(rDiag.length));
		}
	}
}
