	...
	protected void computeCoefficients() throws java.lang.ArithmeticException {
		...
		for (i = 0; i < n; i++) {
			...
			if (d == 0.0) {
				for (int k = 0; k < n; ++k) {
					if ((i != k) && ((x[i]) == (x[k]))) {
						throw org.apache.commons.math.MathRuntimeException.createArithmeticException("identical abscissas x[{0}] == x[{1}] == {2} cause division by zero", new java.lang.Object[]{ i , k , x[i] });
					}
				}
			}
			t = (y[i]) / d;
