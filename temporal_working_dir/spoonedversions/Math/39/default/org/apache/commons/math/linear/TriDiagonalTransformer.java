...
class TriDiagonalTransformer {
	...
	public TriDiagonalTransformer(org.apache.commons.math.linear.RealMatrix matrix) {
		if (!(matrix.isSquare())) {
	...
	public org.apache.commons.math.linear.RealMatrix getT() {
		if ((cachedT) == null) {
			...
			cachedT = org.apache.commons.math.linear.MatrixUtils.createRealMatrix(ta);
		}
		return cachedT;
	}
	...
	private void transform() {
		...
		for (int k = 0; k < (m - 1); k++) {
			...
			final double a = (hK[(k + 1)]) > 0 ? -(org.apache.commons.math.util.FastMath.sqrt(xNormSqr)) : org.apache.commons.math.util.FastMath.sqrt(xNormSqr);
			secondary[k] = a;
			if (a != 0.0) {
				hK[(k + 1)] -= a;
