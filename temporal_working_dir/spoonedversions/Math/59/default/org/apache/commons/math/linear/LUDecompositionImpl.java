...
public class LUDecompositionImpl implements org.apache.commons.math.linear.LUDecomposition {
	...
	public LUDecompositionImpl(org.apache.commons.math.linear.RealMatrix matrix) {
		this(matrix, org.apache.commons.math.linear.LUDecompositionImpl.DEFAULT_TOO_SMALL);
	}

	public LUDecompositionImpl(org.apache.commons.math.linear.RealMatrix matrix, double singularityThreshold) {
		if (!(matrix.isSquare())) {
			throw new org.apache.commons.math.exception.NonSquareMatrixException(matrix.getRowDimension(), matrix.getColumnDimension());
		}
		...
		for (int col = 0; col < m; col++) {
			...
			for (int row = col; row < m; row++) {
				...
				luRow[col] = sum;
				if ((org.apache.commons.math.util.FastMath.abs(sum)) > largest) {
					largest = org.apache.commons.math.util.FastMath.abs(sum);
					max = row;
				}
			}
			if ((org.apache.commons.math.util.FastMath.abs(lu[max][col])) < singularityThreshold) {
				singular = true;
				return ;
			}
			if (max != col) {
	...
	private static class Solver implements org.apache.commons.math.linear.DecompositionSolver {
		...
		public double[] solve(double[] b) {
			final int m = pivot.length;
			if ((b.length) != m) {
				throw new org.apache.commons.math.exception.DimensionMismatchException(b.length, m);
			}
			if (singular) {
				throw new org.apache.commons.math.exception.SingularMatrixException();
			}
		...
		public org.apache.commons.math.linear.ArrayRealVector solve(org.apache.commons.math.linear.ArrayRealVector b) {
