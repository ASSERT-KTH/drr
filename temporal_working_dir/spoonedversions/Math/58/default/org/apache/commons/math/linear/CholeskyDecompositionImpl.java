	...
	public CholeskyDecompositionImpl(final org.apache.commons.math.linear.RealMatrix matrix, final double relativeSymmetryThreshold, final double absolutePositivityThreshold) {
		...
		for (int i = 0; i < order; ++i) {
			...
			for (int j = i + 1; j < order; ++j) {
				...
				if ((org.apache.commons.math.util.FastMath.abs((lIJ - lJI))) > maxDelta) {
					throw new org.apache.commons.math.exception.NonSymmetricMatrixException(i, j, relativeSymmetryThreshold);
				}
				lJ[i] = 0;
			}
		}
		for (int i = 0; i < order; ++i) {
			...
			if ((ltI[i]) < absolutePositivityThreshold) {
				throw new org.apache.commons.math.exception.NonPositiveDefiniteMatrixException(i, absolutePositivityThreshold);
			}
