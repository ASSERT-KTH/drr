...
final class SaddlePointExpansion {
	private static final double HALF_LOG_2_PI = 0.5 * (org.apache.commons.math.util.FastMath.log(org.apache.commons.math.util.MathUtils.TWO_PI));
	...
	static double getStirlingError(double z) {
		...
		if (z < 15.0) {
			double z2 = 2.0 * z;
			if ((org.apache.commons.math.util.FastMath.floor(z2)) == z2) {
				ret = org.apache.commons.math.distribution.SaddlePointExpansion.EXACT_STIRLING_ERRORS[((int) (z2))];
			}else {
				ret = (((org.apache.commons.math.special.Gamma.logGamma((z + 1.0))) - ((z + 0.5) * (org.apache.commons.math.util.FastMath.log(z)))) + z) - (org.apache.commons.math.distribution.SaddlePointExpansion.HALF_LOG_2_PI);
			}
		}else {
			double z2 = z * z;
	...
	static double getDeviancePart(double x, double mu) {
		double ret;
		if ((org.apache.commons.math.util.FastMath.abs((x - mu))) < (0.1 * (x + mu))) {
			double d = x - mu;
			double v = d / (x + mu);
			double s1 = v * d;
			...
			ret = ((x * (org.apache.commons.math.util.FastMath.log((x / mu)))) + mu) - x;
		}
		return ret;
	}
	...
	static double logBinomialProbability(int x, int n, double p, double q) {
		...
		if (x == 0) {
			if (p < 0.1) {
				...
				ret = n * (org.apache.commons.math.util.FastMath.log(q));
			}
		}else
			if (x == n) {
				if (q < 0.1) {
					...
					ret = n * (org.apache.commons.math.util.FastMath.log(p));
				}
			}else {
				...
				ret = ((-0.5) * (org.apache.commons.math.util.FastMath.log(f))) + ret;
			}
