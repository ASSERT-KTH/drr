...
public class MullerSolver extends org.apache.commons.math3.analysis.solvers.AbstractUnivariateSolver {
	private static final double DEFAULT_ABSOLUTE_ACCURACY = 1.0E-6;

	public MullerSolver() {
		this(org.apache.commons.math3.analysis.solvers.MullerSolver.DEFAULT_ABSOLUTE_ACCURACY);
	}
	...
	protected double doSolve() {
		...
		if ((org.apache.commons.math3.util.FastMath.abs(fMin)) < functionValueAccuracy) {
			return min;
		}
		final double fMax = computeObjectiveValue(max);
		if ((org.apache.commons.math3.util.FastMath.abs(fMax)) < functionValueAccuracy) {
			return max;
		}
		final double fInitial = computeObjectiveValue(initial);
		if ((org.apache.commons.math3.util.FastMath.abs(fInitial)) < functionValueAccuracy) {
			return initial;
		}
	...
	private double solve(double min, double max, double fMin, double fMax) {
		...
		while (true) {
			...
			final double xplus = x1 + (((-2.0) * y1) / (c1 + (org.apache.commons.math3.util.FastMath.sqrt(delta))));
			final double xminus = x1 + (((-2.0) * y1) / (c1 - (org.apache.commons.math3.util.FastMath.sqrt(delta))));
			...
			if (!bisect) {
				...
				if (((org.apache.commons.math3.util.FastMath.signum(y0)) + (org.apache.commons.math3.util.FastMath.signum(ym))) == 0.0) {
					x2 = xm;
					y2 = ym;
				}else {
					x0 = xm;
