...
public class PolynomialFunctionLagrangeForm implements java.io.Serializable , org.apache.commons.math.analysis.UnivariateRealFunction {
	...
	public static double evaluate(double[] x, double[] y, double z) throws java.lang.IllegalArgumentException, org.apache.commons.math.FunctionEvaluationException {
		int i;
		int j;
		int n;
		int nearest = 0;
		...
		for (i = 1; i < n; i++) {
			for (j = 0; j < (n - i); j++) {
				...
				divider = (x[j]) - (x[(i + j)]);
				if (divider == 0.0) {
					throw new org.apache.commons.math.FunctionEvaluationException(z, ((((("Identical abscissas cause division by zero: x[" + i) + "] = x[") + (i + j)) + "] = ") + (x[i])));
				}
				w = ((c[(j + 1)]) - (d[j])) / divider;
				c[j] = tc * w;
				d[j] = td * w;
