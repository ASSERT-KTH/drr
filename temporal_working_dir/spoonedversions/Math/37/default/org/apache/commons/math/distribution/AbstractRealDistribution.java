...
public abstract class AbstractRealDistribution implements java.io.Serializable , org.apache.commons.math.distribution.RealDistribution {
	...
	private double solverAbsoluteAccuracy = org.apache.commons.math.distribution.AbstractRealDistribution.SOLVER_DEFAULT_ABSOLUTE_ACCURACY;
	...
	public double inverseCumulativeProbability(final double p) throws org.apache.commons.math.exception.OutOfRangeException {
		...
		final double sig = org.apache.commons.math.util.FastMath.sqrt(getNumericalVariance());
		...
		if (lowerBound == (java.lang.Double.NEGATIVE_INFINITY)) {
			if (chebyshevApplies) {
				lowerBound = mu - (sig * (org.apache.commons.math.util.FastMath.sqrt(((1.0 - p) / p))));
			}else {
				lowerBound = -1.0;
		...
		if (upperBound == (java.lang.Double.POSITIVE_INFINITY)) {
			if (chebyshevApplies) {
				upperBound = mu + (sig * (org.apache.commons.math.util.FastMath.sqrt((p / (1.0 - p)))));
			}else {
				upperBound = 1.0;
		...
		final org.apache.commons.math.analysis.UnivariateFunction toSolve = new org.apache.commons.math.analysis.UnivariateFunction() {
			public double value(final double x) {
				return (cumulativeProbability(x)) - p;
			}
		};
		double x = org.apache.commons.math.analysis.solvers.UnivariateSolverUtils.solve(toSolve, lowerBound, upperBound, getSolverAbsoluteAccuracy());
	...
	public double sample() {
		return randomData.nextInversionDeviate(org.apache.commons.math.distribution.AbstractRealDistribution.this);
	}
