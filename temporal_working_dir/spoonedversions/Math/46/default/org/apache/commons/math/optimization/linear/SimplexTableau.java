...
class SimplexTableau implements java.io.Serializable {
	...
	protected java.lang.Integer getBasicRow(final int col) {
		...
		for (int i = 0; i < (getHeight()); i++) {
			...
			if ((org.apache.commons.math.util.MathUtils.equals(entry, 1.0, maxUlps)) && (row == null)) {
				row = i;
			}else
				if (!(org.apache.commons.math.util.MathUtils.equals(entry, 0.0, maxUlps))) {
					return null;
				}
	...
	boolean isOptimal() {
		for (int i = getNumObjectiveFunctions(); i < ((getWidth()) - 1); i++) {
			final double entry = tableau.getEntry(0, i);
			if ((org.apache.commons.math.util.MathUtils.compareTo(entry, 0.0, epsilon)) < 0) {
				return false;
			}
		}
		return true;
