...
class FastMathCalc {
	...
	private static void buildSinCosTables(double[] SINE_TABLE_A, double[] SINE_TABLE_B, double[] COSINE_TABLE_A, double[] COSINE_TABLE_B, int SINE_TABLE_LEN, double[] TANGENT_TABLE_A, double[] TANGENT_TABLE_B) {
		...
		for (int i = 0; i < 7; i++) {
			double x = i / 8.0;
			org.apache.commons.math3.util.FastMathCalc.slowSin(x, result);
			SINE_TABLE_A[i] = result[0];
			SINE_TABLE_B[i] = result[1];
			org.apache.commons.math3.util.FastMathCalc.slowCos(x, result);
		...
		for (int i = 0; i < SINE_TABLE_LEN; i++) {
			...
			org.apache.commons.math3.util.FastMathCalc.splitReciprocal(as, ys);
			xs[0] = SINE_TABLE_A[i];
			xs[1] = SINE_TABLE_B[i];
	...
	static double slowCos(final double x, final double[] result) {
		...
		org.apache.commons.math3.util.FastMathCalc.split(x, xs);
		ys[0] = ys[1] = 0.0;
		for (int i = (org.apache.commons.math3.util.FastMathCalc.FACT.length) - 1; i >= 0; i--) {
			org.apache.commons.math3.util.FastMathCalc.splitMult(xs, ys, as);
			ys[0] = as[0];
			ys[1] = as[1];
			if ((i & 1) != 0) {
			...
			org.apache.commons.math3.util.FastMathCalc.split(org.apache.commons.math3.util.FastMathCalc.FACT[i], as);
	...
	private static void split(final double d, final double[] split) {
		if ((d < 8.0E298) && (d > (-8.0E298))) {
			final double a = d * (org.apache.commons.math3.util.FastMathCalc.HEX_40000000);
			split[0] = (d + a) - a;
			split[1] = d - (split[0]);
		}else {
			final double a = d * 9.313225746154785E-10;
			split[0] = ((d + a) - d) * (org.apache.commons.math3.util.FastMathCalc.HEX_40000000);
			split[1] = d - (split[0]);
		}
	}
	...
	private static void resplit(final double[] a) {
		...
		if ((c < 8.0E298) && (c > (-8.0E298))) {
			double z = c * (org.apache.commons.math3.util.FastMathCalc.HEX_40000000);
			a[0] = (c + z) - z;
			a[1] = (c - (a[0])) + d;
		}else {
			double z = c * 9.313225746154785E-10;
			a[0] = ((c + z) - c) * (org.apache.commons.math3.util.FastMathCalc.HEX_40000000);
			a[1] = (c - (a[0])) + d;
		}
	}
	...
	private static void splitMult(double[] a, double[] b, double[] ans) {
		...
		org.apache.commons.math3.util.FastMathCalc.resplit(ans);
	}
	...
	private static void quadMult(final double[] a, final double[] b, final double[] result) {
		...
		org.apache.commons.math3.util.FastMathCalc.split(a[0], xs);
		org.apache.commons.math3.util.FastMathCalc.split(b[0], ys);
		org.apache.commons.math3.util.FastMathCalc.splitMult(xs, ys, zs);
		result[0] = zs[0];
		result[1] = zs[1];
		org.apache.commons.math3.util.FastMathCalc.split(b[1], ys);
		org.apache.commons.math3.util.FastMathCalc.splitMult(xs, ys, zs);
		...
		result[0] = tmp;
		org.apache.commons.math3.util.FastMathCalc.split(a[1], xs);
		org.apache.commons.math3.util.FastMathCalc.split(b[0], ys);
		org.apache.commons.math3.util.FastMathCalc.splitMult(xs, ys, zs);
		tmp = (result[0]) + (zs[0]);
		result[1] = (result[1]) - ((tmp - (result[0])) - (zs[0]));
	...
	static double expint(int p, final double[] result) {
		...
		org.apache.commons.math3.util.FastMathCalc.split(1.0, ys);
		while (p > 0) {
			if ((p & 1) != 0) {
	...
	static double[] slowLog(double xi) {
		...
		org.apache.commons.math3.util.FastMathCalc.split(xi, x);
		x[0] += 1.0;
		org.apache.commons.math3.util.FastMathCalc.resplit(x);
		org.apache.commons.math3.util.FastMathCalc.splitReciprocal(x, a);
		x[0] -= 2.0;
		org.apache.commons.math3.util.FastMathCalc.resplit(x);
		org.apache.commons.math3.util.FastMathCalc.splitMult(x, a, y);
		x[0] = y[0];
		x[1] = y[1];
		org.apache.commons.math3.util.FastMathCalc.splitMult(x, x, x2);
		y[0] = org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF[((org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF.length) - 1)][0];
		y[1] = org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF[((org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF.length) - 1)][1];
		for (int i = (org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF.length) - 2; i >= 0; i--) {
			org.apache.commons.math3.util.FastMathCalc.splitMult(y, x2, a);
			y[0] = a[0];
			y[1] = a[1];
			org.apache.commons.math3.util.FastMathCalc.splitAdd(y, org.apache.commons.math3.util.FastMathCalc.LN_SPLIT_COEF[i], a);
			y[0] = a[0];
			y[1] = a[1];
		}
		org.apache.commons.math3.util.FastMathCalc.splitMult(y, x, a);
		y[0] = a[0];
		y[1] = a[1];
		return y;
	}
	...
	static void printarray(java.io.PrintStream out, java.lang.String name, int expectedLen, double[][] array2d) {
		out.println(name);
		org.apache.commons.math3.util.FastMathCalc.checkLen(expectedLen, array2d.length);
		out.println(((org.apache.commons.math3.util.FastMathCalc.TABLE_START_DECL) + " "));
		int i = 0;
		for (double[] array : array2d) {
			out.print("        {");
			for (double d : array) {
				out.printf("%-25.25s", org.apache.commons.math3.util.FastMathCalc.format(d));
			}
			out.println(("}, // " + (i++)));
		}
		out.println(org.apache.commons.math3.util.FastMathCalc.TABLE_END_DECL);
	}

	static void printarray(java.io.PrintStream out, java.lang.String name, int expectedLen, double[] array) {
		out.println((name + "="));
		org.apache.commons.math3.util.FastMathCalc.checkLen(expectedLen, array.length);
		out.println(org.apache.commons.math3.util.FastMathCalc.TABLE_START_DECL);
		for (double d : array) {
			out.printf("        %s%n", org.apache.commons.math3.util.FastMathCalc.format(d));
		}
		out.println(org.apache.commons.math3.util.FastMathCalc.TABLE_END_DECL);
	}
	...
	private static void checkLen(int expectedLen, int actual) throws org.apache.commons.math3.exception.DimensionMismatchException {
