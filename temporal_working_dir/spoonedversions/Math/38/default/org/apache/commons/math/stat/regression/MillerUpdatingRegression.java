...
public class MillerUpdatingRegression implements org.apache.commons.math.stat.regression.UpdatingMultipleLinearRegression {
	...
	public MillerUpdatingRegression(int numberOfVariables, boolean includeConstant) {
		this(numberOfVariables, includeConstant, org.apache.commons.math.util.Precision.EPSILON);
	}
	...
	private void include(final double[] x, final double wi, final double yi) {
		...
		for (int i = 0; i < (x.length); i++) {
			...
			if (di != 0.0) {
				...
				if ((org.apache.commons.math.util.FastMath.abs(tmp)) > (org.apache.commons.math.util.Precision.EPSILON)) {
					w = (di * w) / dpi;
				}
			}else {
				dpi = wxi * xi;
	...
	private double smartAdd(double a, double b) {
		...
		if (_a > _b) {
			double eps = _a * (org.apache.commons.math.util.Precision.EPSILON);
			if (_b > eps) {
				return a + b;
			}
			return a;
		}else {
			double eps = _b * (org.apache.commons.math.util.Precision.EPSILON);
			if (_a > eps) {
				return a + b;
			}
			return b;
