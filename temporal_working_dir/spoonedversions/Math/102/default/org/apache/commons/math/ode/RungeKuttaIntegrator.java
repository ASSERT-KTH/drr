...
public abstract class RungeKuttaIntegrator implements org.apache.commons.math.ode.FirstOrderIntegrator {
	protected RungeKuttaIntegrator(double[] c, double[][] a, double[] b, org.apache.commons.math.ode.RungeKuttaStepInterpolator prototype, double step) {
	...
	private void sanityChecks(org.apache.commons.math.ode.FirstOrderDifferentialEquations equations, double t0, double[] y0, double t, double[] y) throws org.apache.commons.math.ode.IntegratorException {
		if ((equations.getDimension()) != (y0.length)) {
			throw new org.apache.commons.math.ode.IntegratorException(("dimensions mismatch: ODE problem has dimension {0}," + " initial state vector has dimension {1}"), new java.lang.String[]{ java.lang.Integer.toString(equations.getDimension()) , java.lang.Integer.toString(y0.length) });
		}
		if ((equations.getDimension()) != (y.length)) {
			throw new org.apache.commons.math.ode.IntegratorException(("dimensions mismatch: ODE problem has dimension {0}," + " final state vector has dimension {1}"), new java.lang.String[]{ java.lang.Integer.toString(equations.getDimension()) , java.lang.Integer.toString(y.length) });
		}
