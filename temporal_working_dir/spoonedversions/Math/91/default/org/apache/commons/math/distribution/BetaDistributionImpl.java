...
public class BetaDistributionImpl extends org.apache.commons.math.distribution.AbstractContinuousDistribution implements org.apache.commons.math.distribution.BetaDistribution {
	...
	public double density(java.lang.Double x) throws org.apache.commons.math.MathException {
		...
		if ((x < 0) || (x > 1)) {
			...
			if (x == 0) {
				if ((alpha) < 1) {
					throw new org.apache.commons.math.MathException("Cannot compute beta density at 0 when alpha = {0,number}", alpha);
				}
				return 0;
			}else
				if (x == 1) {
					if ((beta) < 1) {
						throw new org.apache.commons.math.MathException("Cannot compute beta density at 1 when beta = %.3g", beta);
					}
					return 0;
				}else {
