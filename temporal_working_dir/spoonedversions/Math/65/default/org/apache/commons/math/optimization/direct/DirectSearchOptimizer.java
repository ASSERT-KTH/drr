...
public abstract class DirectSearchOptimizer implements org.apache.commons.math.optimization.MultivariateRealOptimizer {
	...
	public void setStartConfiguration(final double[] steps) throws java.lang.IllegalArgumentException {
		...
		for (int i = 0; i < n; ++i) {
			...
			for (int j = 0; j < (i + 1); ++j) {
				if ((steps[j]) == 0.0) {
					throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.EQUAL_VERTICES_IN_SIMPLEX, j, (j + 1));
				}
	...
	public void setStartConfiguration(final double[][] referenceSimplex) throws java.lang.IllegalArgumentException {
		...
		if (n < 0) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.SIMPLEX_NEED_ONE_POINT);
		}
		startConfiguration = new double[n][n];
		...
		for (int i = 0; i < (n + 1); ++i) {
			...
			if ((refI.length) != n) {
				throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE, refI.length, n);
			}
			for (int j = 0; j < i; ++j) {
				...
				if (allEquals) {
					throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.EQUAL_VERTICES_IN_SIMPLEX, i, j);
				}
			}
			if (i > 0) {
	...
	private void buildSimplex(final double[] startPoint) throws java.lang.IllegalArgumentException {
		...
		if (n != (startConfiguration.length)) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE, n, startConfiguration.length);
		}
