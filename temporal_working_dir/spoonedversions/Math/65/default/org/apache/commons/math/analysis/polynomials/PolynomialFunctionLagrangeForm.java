...
public class PolynomialFunctionLagrangeForm implements org.apache.commons.math.analysis.UnivariateRealFunction {
	...
	protected void computeCoefficients() throws java.lang.ArithmeticException {
		...
		for (int i = 0; i < n; i++) {
			...
			if (d == 0.0) {
				for (int k = 0; k < n; ++k) {
					if ((i != k) && ((x[i]) == (x[k]))) {
						throw org.apache.commons.math.MathRuntimeException.createArithmeticException(org.apache.commons.math.exception.LocalizedFormats.IDENTICAL_ABSCISSAS_DIVISION_BY_ZERO, i, k, x[i]);
					}
				}
			}
	...
	public static void verifyInterpolationArray(double[] x, double[] y) throws java.lang.IllegalArgumentException {
		if ((x.length) != (y.length)) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE, x.length, y.length);
		}
		if ((x.length) < 2) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.LocalizedFormats.WRONG_NUMBER_OF_POINTS, 2, x.length);
		}
	}
}
