...
public class PolynomialFunctionLagrangeForm implements org.apache.commons.math.analysis.UnivariateRealFunction {
	...
	public double value(double z) throws org.apache.commons.math.FunctionEvaluationException {
		try {
			...
			throw new org.apache.commons.math.FunctionEvaluationException(e, z, e.getLocalizablePattern(), e.getArguments());
		}
	}
	...
	protected void computeCoefficients() throws java.lang.ArithmeticException {
		...
		for (int i = 0; i < n; i++) {
			...
			if (d == 0.0) {
				for (int k = 0; k < n; ++k) {
					if ((i != k) && ((x[i]) == (x[k]))) {
						throw org.apache.commons.math.MathRuntimeException.createArithmeticException(org.apache.commons.math.exception.util.LocalizedFormats.IDENTICAL_ABSCISSAS_DIVISION_BY_ZERO, i, k, x[i]);
					}
				}
			}
	...
	public static void verifyInterpolationArray(double[] x, double[] y) throws java.lang.IllegalArgumentException {
		if ((x.length) != (y.length)) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.DIMENSIONS_MISMATCH_SIMPLE, x.length, y.length);
		}
		if ((x.length) < 2) {
			throw org.apache.commons.math.MathRuntimeException.createIllegalArgumentException(org.apache.commons.math.exception.util.LocalizedFormats.WRONG_NUMBER_OF_POINTS, 2, x.length);
		}
	}
}
