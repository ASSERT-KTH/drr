...
public class GraggBulirschStoerIntegrator extends org.apache.commons.math.ode.nonstiff.AdaptiveStepsizeIntegrator {
	...
	@java.lang.Override
	public void addEventHandler(final org.apache.commons.math.ode.events.EventHandler function, final double maxCheckInterval, final double convergence, final int maxIterationCount, final org.apache.commons.math.analysis.solvers.UnivariateRealSolver solver) {
	...
	private void rescale(final double[] y1, final double[] y2, final double[] scale) {
		if ((vecAbsoluteTolerance) == null) {
			for (int i = 0; i < (scale.length); ++i) {
				final double yi = org.apache.commons.math.util.FastMath.max(org.apache.commons.math.util.FastMath.abs(y1[i]), org.apache.commons.math.util.FastMath.abs(y2[i]));
	...
	private boolean tryStep(final double t0, final double[] y0, final double step, final int k, final double[] scale, final double[][] f, final double[] yMiddle, final double[] yEnd, final double[] yTmp) throws org.apache.commons.math.exception.MathUserException {
		...
		for (int j = 1; j < n; ++j) {
			...
			if (((performTest) && (j <= (maxChecks))) && (k < (maxIter))) {
				...
				if (deltaNorm > (4 * (org.apache.commons.math.util.FastMath.max(1.0E-15, initialNorm)))) {
					return false;
				}
			}
		}
	...
	public double integrate(final org.apache.commons.math.ode.FirstOrderDifferentialEquations equations, final double t0, final double[] y0, final double t, final double[] y) throws org.apache.commons.math.exception.MathIllegalArgumentException, org.apache.commons.math.exception.MathIllegalStateException {
		...
		final double log10R = org.apache.commons.math.util.FastMath.log10(org.apache.commons.math.util.FastMath.max(1.0E-10, tol));
		int targetIter = org.apache.commons.math.util.FastMath.max(1, org.apache.commons.math.util.FastMath.min(((sequence.length) - 2), ((int) (org.apache.commons.math.util.FastMath.floor((0.5 - (0.6 * log10R)))))));
		final org.apache.commons.math.ode.sampling.AbstractStepInterpolator interpolator = new org.apache.commons.math.ode.nonstiff.GraggBulirschStoerStepInterpolator(y, yDot0, y1, yDot1, yMidDots, forward);
		...
		do {
			...
			for (boolean loop = true; loop;) {
				...
				if (!(tryStep(stepStart, y, stepSize, k, scale, fk[k], (k == 0 ? yMidDots[0] : diagonal[(k - 1)]), (k == 0 ? y1 : y1Diag[(k - 1)]), yTmp))) {
					hNew = org.apache.commons.math.util.FastMath.abs(filterStep(((stepSize) * (stabilityReduction)), forward, false));
					reject = true;
					loop = false;
				}else {
					if (k > 0) {
						...
						for (int j = 0; j < (mainSetDimension); ++j) {
							final double e = (org.apache.commons.math.util.FastMath.abs(((y1[j]) - (y1Diag[0][j])))) / (scale[j]);
							error += e * e;
						}
						...
						if ((error > 1.0E15) || ((k > 1) && (error > maxError))) {
							...
							maxError = org.apache.commons.math.util.FastMath.max((4 * error), 1.0);
							final double exp = 1.0 / ((2 * k) + 1);
							double fac = (stepControl2) / (org.apache.commons.math.util.FastMath.pow((error / (stepControl1)), exp));
							final double pow = org.apache.commons.math.util.FastMath.pow(stepControl3, exp);
							fac = org.apache.commons.math.util.FastMath.max((pow / (stepControl4)), org.apache.commons.math.util.FastMath.min((1 / pow), fac));
							optimalStep[k] = org.apache.commons.math.util.FastMath.abs(filterStep(((stepSize) * fac), forward, true));
			...
			if (!reject) {
				...
				for (int l = 0; l < mu; ++l) {
					final int l2 = l / 2;
					double factor = org.apache.commons.math.util.FastMath.pow((0.5 * (sequence[l2])), l);
					int middleIndex = (fk[l2].length) / 2;
					for (int i = 0; i < (y0.length); ++i) {
						yMidDots[(l + 1)][i] = factor * (fk[l2][(middleIndex + l)][i]);
					}
					for (int j = 1; j <= (k - l2); ++j) {
						factor = org.apache.commons.math.util.FastMath.pow((0.5 * (sequence[(j + l2)])), l);
				...
				if (mu >= 0) {
					...
					if (useInterpolationError) {
						...
						hInt = org.apache.commons.math.util.FastMath.abs(((stepSize) / (org.apache.commons.math.util.FastMath.max(org.apache.commons.math.util.FastMath.pow(interpError, (1.0 / (mu + 4))), 0.01))));
						if (interpError > 10.0) {
							hNew = hInt;
							reject = true;
						}
					...
					if (k <= targetIter) {
						...
						if ((costPerTimeUnit[(k - 1)]) < ((orderControl1) * (costPerTimeUnit[k]))) {
							...
							if ((costPerTimeUnit[k]) < ((orderControl2) * (costPerTimeUnit[(k - 1)]))) {
								optimalIter = org.apache.commons.math.util.FastMath.min((k + 1), ((sequence.length) - 2));
							}
						...
						if ((costPerTimeUnit[k]) < ((orderControl2) * (costPerTimeUnit[optimalIter]))) {
							optimalIter = org.apache.commons.math.util.FastMath.min(k, ((sequence.length) - 2));
						}
					}
				
				if (previousRejected) {
					targetIter = org.apache.commons.math.util.FastMath.min(optimalIter, k);
					hNew = org.apache.commons.math.util.FastMath.min(org.apache.commons.math.util.FastMath.abs(stepSize), optimalStep[targetIter]);
				}else {
					if (optimalIter <= k) {
			...
			hNew = org.apache.commons.math.util.FastMath.min(hNew, hInt);
			if (!forward) {
				hNew = -hNew;
			}
			firstTime = false;
