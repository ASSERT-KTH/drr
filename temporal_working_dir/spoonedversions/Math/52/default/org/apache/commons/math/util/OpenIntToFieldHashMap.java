...
public class OpenIntToFieldHashMap<T extends org.apache.commons.math.FieldElement<T>> implements java.io.Serializable {
	...
	public OpenIntToFieldHashMap(final org.apache.commons.math.Field<T> field) {
		this(field, org.apache.commons.math.util.OpenIntToFieldHashMap.DEFAULT_EXPECTED_SIZE, field.getZero());
	}

	public OpenIntToFieldHashMap(final org.apache.commons.math.Field<T> field, final T missingEntries) {
		this(field, org.apache.commons.math.util.OpenIntToFieldHashMap.DEFAULT_EXPECTED_SIZE, missingEntries);
	}
	...
	private static int computeCapacity(final int expectedSize) {
		...
		final int capacity = ((int) (org.apache.commons.math.util.FastMath.ceil((expectedSize / (org.apache.commons.math.util.OpenIntToFieldHashMap.LOAD_FACTOR)))));
	...
	public T get(final int key) {
		...
		int j = index;
		for (int perturb = org.apache.commons.math.util.OpenIntToFieldHashMap.perturb(hash); (states[index]) != (org.apache.commons.math.util.OpenIntToFieldHashMap.FREE); perturb >>= org.apache.commons.math.util.OpenIntToFieldHashMap.PERTURB_SHIFT) {
	...
	private boolean containsKey(final int key, final int index) {
		return ((key != 0) || ((states[index]) == (org.apache.commons.math.util.OpenIntToFieldHashMap.FULL))) && ((keys[index]) == key);
	}

	private T doRemove(int index) {
		keys[index] = 0;
		states[index] = org.apache.commons.math.util.OpenIntToFieldHashMap.REMOVED;
	...
	public T put(final int key, final T value) {
		...
		keys[index] = key;
		states[index] = org.apache.commons.math.util.OpenIntToFieldHashMap.FULL;
		values[index] = value;
		if (newMapping) {
			++(size);
			if (shouldGrowTable()) {
	...
	private void growTable() {
		...
		final int newLength = (org.apache.commons.math.util.OpenIntToFieldHashMap.RESIZE_MULTIPLIER) * oldLength;
	...
	private boolean shouldGrowTable() {
		return (size) > (((mask) + 1) * (org.apache.commons.math.util.OpenIntToFieldHashMap.LOAD_FACTOR));
	}
	...
	public class Iterator {
		...
		public int key() throws java.util.ConcurrentModificationException, java.util.NoSuchElementException {
			if ((referenceCount) != (count)) {
				throw org.apache.commons.math.MathRuntimeException.createConcurrentModificationException(org.apache.commons.math.exception.util.LocalizedFormats.MAP_MODIFIED_WHILE_ITERATING);
			}
			if ((current) < 0) {
				throw org.apache.commons.math.MathRuntimeException.createNoSuchElementException(org.apache.commons.math.exception.util.LocalizedFormats.ITERATOR_EXHAUSTED);
			}
			return keys[current];
		}
