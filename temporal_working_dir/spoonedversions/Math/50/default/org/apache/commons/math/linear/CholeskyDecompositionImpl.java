...
public class CholeskyDecompositionImpl implements org.apache.commons.math.linear.CholeskyDecomposition {
	...
	public CholeskyDecompositionImpl(final org.apache.commons.math.linear.RealMatrix matrix) {
	...
	public CholeskyDecompositionImpl(final org.apache.commons.math.linear.RealMatrix matrix, final double relativeSymmetryThreshold, final double absolutePositivityThreshold) {
		if (!(matrix.isSquare())) {
		...
		for (int i = 0; i < order; ++i) {
			...
			if ((ltI[i]) < absolutePositivityThreshold) {
				throw new org.apache.commons.math.linear.NonPositiveDefiniteMatrixException(i, absolutePositivityThreshold);
			}
	...
	private static class Solver implements org.apache.commons.math.linear.DecompositionSolver {
		...
		private double[][] solve(double[][] b, boolean reuseB) {
			...
			if (reuseB) {
				...
				for (int i = 0; i < (b.length); ++i) {
					java.lang.System.arraycopy(b[i], 0, x[i], 0, nColB);
				}
			}
			for (int j = 0; j < m; j++) {
				final double[] lJ = lTData[j];
		...
		public org.apache.commons.math.linear.RealMatrix solve(org.apache.commons.math.linear.RealMatrix b) {
			return new org.apache.commons.math.linear.Array2DRowRealMatrix(solve(b.getData(), true), false);
		}
