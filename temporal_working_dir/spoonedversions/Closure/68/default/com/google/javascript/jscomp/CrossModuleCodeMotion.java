...
class CrossModuleCodeMotion extends com.google.javascript.jscomp.NodeTraversal.AbstractPostOrderCallback implements com.google.javascript.jscomp.CompilerPass {
	...
	private void processReference(com.google.javascript.jscomp.NodeTraversal t, com.google.javascript.jscomp.CrossModuleCodeMotion.NamedInfo info, java.lang.String name) {
		...
		if ((rootNode.getType()) == (com.google.javascript.rhino.Token.FUNCTION)) {
			...
			if (scopeFuncName.equals(name)) {
				...
				if (((scopeFuncParent.getType()) == (com.google.javascript.rhino.Token.NAME)) && (scopeFuncParent.getString().equals(name))) {
					recursive = true;
				}else {
					for (com.google.javascript.jscomp.Scope s = t.getScope(); (s.getParent()) != null; s = s.getParent()) {
						...
						if ((curRoot.getParent().getType()) == (com.google.javascript.rhino.Token.ASSIGN)) {
							com.google.javascript.rhino.Node owner = curRoot.getParent().getFirstChild();
							while ((owner.getType()) == (com.google.javascript.rhino.Token.GETPROP)) {
								owner = owner.getFirstChild();
							} 
							if (((owner.getType()) == (com.google.javascript.rhino.Token.NAME)) && (owner.getString().equals(name))) {
								recursive = true;
								break;
							}
						}
		...
		switch (parent.getType()) {
			...
			case com.google.javascript.rhino.Token.GETPROP :
				...
				for (com.google.javascript.rhino.Node current : name.getAncestors()) {
					if ((current.getType()) == (com.google.javascript.rhino.Token.GETPROP)) {
					}else
						if (((current.getType()) == (com.google.javascript.rhino.Token.ASSIGN)) && ((current.getFirstChild()) == child)) {
	...
	private boolean canMoveValue(com.google.javascript.rhino.Node n) {
		if (((n == null) || (com.google.javascript.jscomp.NodeUtil.isLiteralValue(n, true))) || ((n.getType()) == (com.google.javascript.rhino.Token.FUNCTION))) {
			return true;
		}else
			if ((n.getType()) == (com.google.javascript.rhino.Token.CALL)) {
				com.google.javascript.rhino.Node functionName = n.getFirstChild();
				return ((functionName.getType()) == (com.google.javascript.rhino.Token.NAME)) && ((functionName.getString().equals(com.google.javascript.jscomp.CrossModuleMethodMotion.STUB_METHOD_NAME)) || (functionName.getString().equals(com.google.javascript.jscomp.CrossModuleMethodMotion.UNSTUB_METHOD_NAME)));
			}else
				if (((n.getType()) == (com.google.javascript.rhino.Token.ARRAYLIT)) || ((n.getType()) == (com.google.javascript.rhino.Token.OBJECTLIT))) {
					boolean isObjectLit = (n.getType()) == (com.google.javascript.rhino.Token.OBJECTLIT);
