		...
		private void checkVar(com.google.javascript.jscomp.NodeTraversal t, com.google.javascript.jscomp.Scope.Var v, java.util.List<com.google.javascript.jscomp.ReferenceCollectingCallback.Reference> references) {
			...
			for (com.google.javascript.jscomp.ReferenceCollectingCallback.Reference reference : references) {
				if (reference.isHoistedFunction()) {
					...
					if (com.google.javascript.jscomp.NodeUtil.isFunctionDeclaration(reference.getNode().getParent())) {
			...
			for (com.google.javascript.jscomp.ReferenceCollectingCallback.Reference reference : references) {
				...
				boolean allowDupe = com.google.javascript.jscomp.SyntacticScopeCreator.hasDuplicateDeclarationSuppression(reference.getNode(), v);
				if (isDeclaration && (!allowDupe)) {
					for (com.google.javascript.jscomp.ReferenceCollectingCallback.BasicBlock declaredBlock : blocksWithDeclarations) {
						if (declaredBlock.provablyExecutesBefore(basicBlock)) {
							java.lang.String filename = com.google.javascript.jscomp.NodeUtil.getSourceName(reference.getNode());
							compiler.report(com.google.javascript.jscomp.JSError.make(filename, reference.getNode(), checkLevel, com.google.javascript.jscomp.VariableReferenceCheck.REDECLARED_VARIABLE, v.name));
							break;
						}
					}
				}
				if ((isUnhoistedNamedFunction && (!isDeclaration)) && isDeclaredInScope) {
					for (com.google.javascript.jscomp.ReferenceCollectingCallback.BasicBlock declaredBlock : blocksWithDeclarations) {
						if (!(declaredBlock.provablyExecutesBefore(basicBlock))) {
							java.lang.String filename = com.google.javascript.jscomp.NodeUtil.getSourceName(reference.getNode());
							compiler.report(com.google.javascript.jscomp.JSError.make(filename, reference.getNode(), com.google.javascript.jscomp.VariableReferenceCheck.AMBIGUOUS_FUNCTION_DECL, v.name));
							break;
						}
					}
				}
				if ((!isDeclaration) && (!isDeclaredInScope)) {
					if (!(reference.getNode().isFromExterns())) {
						...
						if ((reference.getScope()) == (v.scope)) {
							java.lang.String filename = com.google.javascript.jscomp.NodeUtil.getSourceName(reference.getNode());
							compiler.report(com.google.javascript.jscomp.JSError.make(filename, reference.getNode(), checkLevel, com.google.javascript.jscomp.VariableReferenceCheck.UNDECLARED_REFERENCE, v.name));
						}
					}
				}
				if (isDeclaration) {
