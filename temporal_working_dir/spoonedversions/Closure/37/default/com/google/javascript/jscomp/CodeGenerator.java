	...
	void add(com.google.javascript.rhino.Node n, com.google.javascript.jscomp.CodeGenerator.Context context) {
		...
		switch (type) {
			...
			case com.google.javascript.rhino.Token.COMMA :
				com.google.common.base.Preconditions.checkState((childCount == 2));
				com.google.javascript.rhino.Node firstNonComma = n.getFirstChild();
				while ((firstNonComma.getType()) == (com.google.javascript.rhino.Token.COMMA)) {
					firstNonComma = firstNonComma.getFirstChild();
				} 
				addExpr(firstNonComma, 0, context);
				com.google.javascript.rhino.Node current = firstNonComma;
				do {
					current = current.getParent();
					cc.listSeparator();
					addExpr(current.getFirstChild().getNext(), 0, com.google.javascript.jscomp.CodeGenerator.Context.OTHER);
				} while (current != n );
				break;
