...
class InlineSimpleMethods extends com.google.javascript.jscomp.MethodCompilerPass {
			...
			if (((definitions.size()) == 1) || (allDefinitionsEquivalent(definitions))) {
				if (!(argsMayHaveSideEffects(callNode))) {
					...
					if (returned != null) {
						...
						if ((com.google.javascript.jscomp.InlineSimpleMethods.isEmptyMethod(firstDefinition)) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(callNode.getFirstChild())))) {
	...
	private static boolean isPropertyTree(com.google.javascript.rhino.Node expectedGetprop) {
		...
		if (((leftChild.getType()) != (com.google.javascript.rhino.Token.THIS)) && (!(com.google.javascript.jscomp.InlineSimpleMethods.isPropertyTree(leftChild)))) {
			return false;
		}
