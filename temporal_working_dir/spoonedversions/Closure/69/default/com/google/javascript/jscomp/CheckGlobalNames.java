...
class CheckGlobalNames implements com.google.javascript.jscomp.CompilerPass {
		...
		for (com.google.javascript.jscomp.GlobalNamespace.Ref ref : name.getRefs()) {
			if ((!isDefined) && (!(isTypedef(ref)))) {
				...
				if (((declaration != null) && ((ref.getModule()) != (declaration.getModule()))) && (!(moduleGraph.dependsOn(ref.getModule(), declaration.getModule())))) {
					...
					if (((ref.scope.isGlobal()) && singleGlobalParentDecl) && ((parent.declaration.preOrderIndex) > (ref.preOrderIndex))) {
						compiler.report(com.google.javascript.jscomp.JSError.make(ref.source.getName(), ref.node, com.google.javascript.jscomp.CheckGlobalNames.NAME_DEFINED_LATE_WARNING, name.fullName(), parent.fullName(), parent.declaration.source.getName(), java.lang.String.valueOf(parent.declaration.node.getLineno())));
					}
	...
	private void reportBadModuleReference(com.google.javascript.jscomp.GlobalNamespace.Name name, com.google.javascript.jscomp.GlobalNamespace.Ref ref) {
		compiler.report(com.google.javascript.jscomp.JSError.make(ref.source.getName(), ref.node, com.google.javascript.jscomp.CheckGlobalNames.STRICT_MODULE_DEP_QNAME, ref.getModule().getName(), name.declaration.getModule().getName(), name.fullName()));
	}
