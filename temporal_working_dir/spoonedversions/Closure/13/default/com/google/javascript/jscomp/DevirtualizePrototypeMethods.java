...
class DevirtualizePrototypeMethods implements com.google.javascript.jscomp.OptimizeCalls.CallGraphCompilerPass , com.google.javascript.jscomp.SpecializationAwareCompilerPass {
	...
	private static boolean isPrototypeMethodDefinition(com.google.javascript.rhino.Node node) {
		...
		if (node.isGetProp()) {
			...
			if (!(com.google.javascript.jscomp.NodeUtil.isExprAssign(gramp))) {
				return false;
			}
			...
			if (node.isStringKey()) {
				...
				com.google.javascript.rhino.Node greatGramp = gramp.getParent();
				if ((greatGramp == null) || (!(greatGramp.isExprResult()))) {
					return false;
				}
				com.google.javascript.rhino.Node functionNode = node.getFirstChild();
				if ((functionNode == null) || (!(functionNode.isFunction()))) {
					return false;
				}
				com.google.javascript.rhino.Node target = gramp.getFirstChild();
	...
	private java.lang.String getMethodName(com.google.javascript.rhino.Node node) {
		if (node.isGetProp()) {
	...
	private java.lang.String getRewrittenMethodName(java.lang.String originalMethodName) {
	...
	private void rewriteDefinitionIfEligible(com.google.javascript.jscomp.DefinitionSite defSite, com.google.javascript.jscomp.SimpleDefinitionFinder defFinder) {
		...
		java.lang.String newMethodName = getRewrittenMethodName(getMethodName(node));
	...
	private void rewriteDefinition(com.google.javascript.rhino.Node node, java.lang.String newMethodName) {
		...
		com.google.javascript.rhino.Node parent = node.getParent();
		com.google.javascript.rhino.Node refNode = isObjLitDefKey ? node : parent.getFirstChild();
		com.google.javascript.rhino.Node newNameNode = com.google.javascript.rhino.IR.name(newMethodName).copyInformationFrom(refNode);
		com.google.javascript.rhino.Node newVarNode = com.google.javascript.rhino.IR.var(newNameNode).copyInformationFrom(refNode);
		...
		if (!isObjLitDefKey) {
			...
			com.google.javascript.rhino.Node assign = parent.getParent();
			com.google.javascript.rhino.Node expr = assign.getParent();
			com.google.javascript.rhino.Node block = expr.getParent();
