...
public class PeepholeSubstituteAlternateSyntax extends com.google.javascript.jscomp.AbstractPeepholeOptimization {
	...
	private com.google.javascript.rhino.Node tryReduceReturn(com.google.javascript.rhino.Node n) {
		...
		boolean possibleException = (result != null) && (com.google.javascript.jscomp.ControlFlowAnalysis.mayThrowException(result));
		...
		while ((follow != null) && (com.google.javascript.jscomp.NodeUtil.isTryFinallyNode(follow.getParent(), follow))) {
			if ((result != null) && ((com.google.javascript.jscomp.NodeUtil.canBeSideEffected(result)) || (com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(result)))) {
				safe = false;
				break;
			}
			follow = com.google.javascript.jscomp.ControlFlowAnalysis.computeFollowNode(follow);
		} 
		if (safe) {
			if (follow == null) {
				if (result == null) {
					n.setType(com.google.javascript.rhino.Token.BREAK);
					reportCodeChange();
					return n;
				}
			}else
				if (((follow.getType()) == (com.google.javascript.rhino.Token.RETURN)) && ((result == (follow.getFirstChild())) || ((((result != null) && (follow.hasChildren())) && (result.checkTreeEqualsSilent(follow.getFirstChild()))) && ((com.google.javascript.jscomp.ControlFlowAnalysis.getExceptionHandler(n)) == (com.google.javascript.jscomp.ControlFlowAnalysis.getExceptionHandler(follow)))))) {
					n.removeChildren();
					n.setType(com.google.javascript.rhino.Token.BREAK);
					reportCodeChange();
					return n;
		...
		if (thenBranchIsExpressionBlock && elseBranchIsExpressionBlock) {
			...
			if ((thenOp.getType()) == (elseOp.getType())) {
				if (com.google.javascript.jscomp.NodeUtil.isAssignmentOp(thenOp)) {
					...
					if (com.google.javascript.jscomp.NodeUtil.isCall(thenOp)) {
						n.removeChild(cond);
	...
	private com.google.javascript.rhino.Node tryMinimizeCondition(com.google.javascript.rhino.Node n) {
		...
		switch (n.getType()) {
					...
					case com.google.javascript.rhino.Token.OR :
						{
							...
							if (((leftParent.getType()) == (com.google.javascript.rhino.Token.NOT)) && ((rightParent.getType()) == (com.google.javascript.rhino.Token.NOT))) {
								com.google.javascript.rhino.Node left = leftParent.removeFirstChild();
								com.google.javascript.rhino.Node right = rightParent.removeFirstChild();
			...
			case com.google.javascript.rhino.Token.AND :
				{
					...
					com.google.javascript.rhino.jstype.TernaryValue rightVal = com.google.javascript.jscomp.NodeUtil.getBooleanValue(right);
					if ((com.google.javascript.jscomp.NodeUtil.getBooleanValue(right)) != (com.google.javascript.rhino.jstype.TernaryValue.UNKNOWN)) {
						int type = n.getType();
			...
			case com.google.javascript.rhino.Token.HOOK :
				{
					...
					com.google.javascript.rhino.Node replacement = null;
					if (((com.google.javascript.jscomp.NodeUtil.getBooleanValue(trueNode)) == (com.google.javascript.rhino.jstype.TernaryValue.TRUE)) && ((com.google.javascript.jscomp.NodeUtil.getBooleanValue(falseNode)) == (com.google.javascript.rhino.jstype.TernaryValue.FALSE))) {
						condition.detachFromParent();
						replacement = condition;
					}else
						if (((com.google.javascript.jscomp.NodeUtil.getBooleanValue(trueNode)) == (com.google.javascript.rhino.jstype.TernaryValue.FALSE)) && ((com.google.javascript.jscomp.NodeUtil.getBooleanValue(falseNode)) == (com.google.javascript.rhino.jstype.TernaryValue.TRUE))) {
							condition.detachFromParent();
							replacement = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.NOT, condition);
						}else
							if ((com.google.javascript.jscomp.NodeUtil.getBooleanValue(trueNode)) == (com.google.javascript.rhino.jstype.TernaryValue.TRUE)) {
								n.detachChildren();
								replacement = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.OR, condition, falseNode);
							}else
								if ((com.google.javascript.jscomp.NodeUtil.getBooleanValue(falseNode)) == (com.google.javascript.rhino.jstype.TernaryValue.FALSE)) {
									n.detachChildren();
			...
			default :
				com.google.javascript.rhino.jstype.TernaryValue nVal = com.google.javascript.jscomp.NodeUtil.getBooleanValue(n);
...
NOT_SAFE_TO_FOLD, SAFE_TO_FOLD_WITH_ARGS, SAFE_TO_FOLD_WITHOUT_ARGS;	}
	...
	private com.google.javascript.rhino.Node tryFoldRegularExpressionConstructor(com.google.javascript.rhino.Node n) {
		...
		if ((((((pattern.getType()) == (com.google.javascript.rhino.Token.STRING)) && (!("".equals(pattern.getString())))) && ((pattern.getString().length()) < 100)) && ((null == flags) || ((flags.getType()) == (com.google.javascript.rhino.Token.STRING)))) && (!(com.google.javascript.jscomp.PeepholeSubstituteAlternateSyntax.containsUnicodeEscape(pattern.getString())))) {
			...
			if ((null == flags) || ("".equals(flags.getString()))) {
				...
				if (!(com.google.javascript.jscomp.PeepholeSubstituteAlternateSyntax.areSafeFlagsToFold(flags.getString()))) {
					return n;
				}
				n.removeChild(flags);
	...
	private static boolean areSafeFlagsToFold(java.lang.String flags) {
