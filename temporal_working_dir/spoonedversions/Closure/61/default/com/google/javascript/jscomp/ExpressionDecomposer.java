...
UNDECOMPOSABLE, MOVABLE, DECOMPOSABLE;	}
	...
	void maybeDecomposeExpression(com.google.javascript.rhino.Node expression) {
		int i = 0;
	...
	void moveExpression(com.google.javascript.rhino.Node expression) {
		java.lang.String resultName = getTempValueName();
		...
		expressionParent.replaceChild(expression, com.google.javascript.rhino.Node.newString(com.google.javascript.rhino.Token.NAME, resultName));
	...
	private void exposeExpression(com.google.javascript.rhino.Node expressionRoot, com.google.javascript.rhino.Node subExpression) {
		...
		for (com.google.javascript.rhino.Node child = nonconditionalExpr, parent = child.getParent(); parent != expressionRoot; child = parent , parent = child.getParent()) {
			...
			if (parentType == (com.google.javascript.rhino.Token.ASSIGN)) {
				...
				if ((parentType == (com.google.javascript.rhino.Token.CALL)) && (com.google.javascript.jscomp.NodeUtil.isGet(parent.getFirstChild()))) {
					if (!(maybeExternMethod(parent.getFirstChild()))) {
						throw new java.lang.IllegalStateException("External object method calls can not be decomposed.");
					}else {
		...
		if (nonconditionalExpr == subExpression) {
			...
			boolean needResult = (parent.getType()) != (com.google.javascript.rhino.Token.EXPR_RESULT);
	...
	private com.google.javascript.rhino.Node extractConditional(com.google.javascript.rhino.Node expr, com.google.javascript.rhino.Node injectionPoint, boolean needResult) {
		...
		com.google.javascript.rhino.Node trueExpr = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.BLOCK).copyInformationFrom(expr);
		com.google.javascript.rhino.Node falseExpr = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.BLOCK).copyInformationFrom(expr);
		switch (expr.getType()) {
		...
		if (falseExpr.hasChildren()) {
			ifNode = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.IF, cond, trueExpr, falseExpr);
		}else {
			ifNode = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.IF, cond, trueExpr);
		}
		...
		if (needResult) {
			...
			com.google.javascript.rhino.Node replacementValueNode = com.google.javascript.rhino.Node.newString(com.google.javascript.rhino.Token.NAME, tempName);
			parent.replaceChild(expr, replacementValueNode);
		}else {
			com.google.common.base.Preconditions.checkArgument(((parent.getType()) == (com.google.javascript.rhino.Token.EXPR_RESULT)));
	...
	private static com.google.javascript.rhino.Node buildResultExpression(com.google.javascript.rhino.Node expr, boolean needResult, java.lang.String tempName) {
		if (needResult) {
			return new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.ASSIGN, com.google.javascript.rhino.Node.newString(com.google.javascript.rhino.Token.NAME, tempName), expr).copyInformationFromForTree(expr);
		}else {
			return expr;
		}
	}
	...
	private boolean isConstantName(com.google.javascript.rhino.Node n, java.util.Set<java.lang.String> knownConstants) {
		return (com.google.javascript.jscomp.NodeUtil.isName(n)) && ((com.google.javascript.jscomp.NodeUtil.isConstantName(n)) || (knownConstants.contains(n.getString())));
	}
	...
	private com.google.javascript.rhino.Node extractExpression(com.google.javascript.rhino.Node expr, com.google.javascript.rhino.Node injectionPoint) {
		com.google.javascript.rhino.Node parent = expr.getParent();
		boolean isLhsOfAssignOp = ((com.google.javascript.jscomp.NodeUtil.isAssignmentOp(parent)) && (!(com.google.javascript.jscomp.NodeUtil.isAssign(parent)))) && ((parent.getFirstChild()) == expr);
		...
		if (isLhsOfAssignOp && (com.google.javascript.jscomp.NodeUtil.isGet(expr))) {
			for (com.google.javascript.rhino.Node n : expr.children()) {
				if (((n.getType()) != (com.google.javascript.rhino.Token.STRING)) && (!(isConstantName(n, knownConstants)))) {
		...
		com.google.javascript.rhino.Node replacementValueNode = com.google.javascript.rhino.Node.newString(com.google.javascript.rhino.Token.NAME, tempName).copyInformationFrom(expr);
		com.google.javascript.rhino.Node tempNameValue;
		if (isLhsOfAssignOp) {
			com.google.common.base.Preconditions.checkState(((com.google.javascript.jscomp.NodeUtil.isName(expr)) || (com.google.javascript.jscomp.NodeUtil.isGet(expr))));
	...
	private com.google.javascript.rhino.Node rewriteCallExpression(com.google.javascript.rhino.Node call, com.google.javascript.jscomp.ExpressionDecomposer.DecompositionState state) {
		...
		com.google.javascript.rhino.Node newCall = new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.CALL, new com.google.javascript.rhino.Node(com.google.javascript.rhino.Token.GETPROP, functionNameNode.cloneNode(), com.google.javascript.rhino.Node.newString("call")), thisNameNode.cloneNode(), call.getLineno(), call.getCharno());
	...
	public void setTempNamePrefix(java.lang.String tempNamePrefix) {
		com.google.javascript.jscomp.ExpressionDecomposer.this.tempNamePrefix = tempNamePrefix;
	}
	...
	static com.google.javascript.rhino.Node findInjectionPoint(com.google.javascript.rhino.Node subExpression) {
		...
		while ((parent.getType()) == (com.google.javascript.rhino.Token.LABEL)) {
		...
		for (com.google.javascript.rhino.Node parent : child.getAncestors()) {
			...
			if (com.google.javascript.jscomp.ExpressionDecomposer.isConditionalOp(parent)) {
				...
				if (isSafeAssign(parent, seenSideEffects)) {
					...
					if ((requiresDecomposition && ((parent.getType()) == (com.google.javascript.rhino.Token.CALL))) && (com.google.javascript.jscomp.NodeUtil.isGet(first))) {
