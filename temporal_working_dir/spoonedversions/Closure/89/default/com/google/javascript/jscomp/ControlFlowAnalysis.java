...
final class ControlFlowAnalysis implements com.google.javascript.jscomp.CompilerPass , com.google.javascript.jscomp.NodeTraversal.Callback {
	...
	ControlFlowAnalysis(com.google.javascript.jscomp.AbstractCompiler compiler, boolean shouldTraverseFunctions) {
	...
	public void process(com.google.javascript.rhino.Node externs, com.google.javascript.rhino.Node root) {
		...
		cfg = new com.google.javascript.jscomp.ControlFlowAnalysis.AstControlFlowGraph(com.google.javascript.jscomp.ControlFlowAnalysis.computeFallThrough(root), nodePriorities);
	...
	private void handleCase(com.google.javascript.rhino.Node node) {
		...
		if (next != null) {
			com.google.common.base.Preconditions.checkState(((next.getType()) == (com.google.javascript.rhino.Token.CASE)));
			...
			com.google.javascript.rhino.Node deflt = com.google.javascript.jscomp.ControlFlowAnalysis.getNextSiblingOfType(parent.getFirstChild().getNext(), com.google.javascript.rhino.Token.DEFAULT);
			if (deflt != null) {
	...
	private void handleStmtList(com.google.javascript.rhino.Node node) {
		com.google.javascript.rhino.Node parent = node.getParent();
		if ((((((node.getType()) == (com.google.javascript.rhino.Token.BLOCK)) && (parent != null)) && ((parent.getType()) == (com.google.javascript.rhino.Token.TRY))) && ((com.google.javascript.jscomp.NodeUtil.getCatchBlock(parent)) == node)) && (!(com.google.javascript.jscomp.NodeUtil.hasCatchHandler(node)))) {
			return ;
		}
		com.google.javascript.rhino.Node child = node.getFirstChild();
		while ((child != null) && ((child.getType()) == (com.google.javascript.rhino.Token.FUNCTION))) {
			child = child.getNext();
		} 
		if (child != null) {
		...
		if (parent != null) {
			switch (parent.getType()) {
				...
				case com.google.javascript.rhino.Token.TRY :
					break;
				default :
					if (((node.getType()) == (com.google.javascript.rhino.Token.BLOCK)) && (node.isSyntheticBlock())) {
	...
	private void handleBreak(com.google.javascript.rhino.Node node) {
		...
		for (cur = node, lastJump = node; !(com.google.javascript.jscomp.ControlFlowAnalysis.isBreakTarget(cur, label)); cur = parent , parent = parent.getParent()) {
			if (((cur.getType()) == (com.google.javascript.rhino.Token.TRY)) && (com.google.javascript.jscomp.NodeUtil.hasFinally(cur))) {
				if (lastJump == node) {
			...
			com.google.common.base.Preconditions.checkState((parent != null), "Cannot find break target.");
		}
		if (lastJump == node) {
	...
	private void handleReturn(com.google.javascript.rhino.Node node) {
		...
		for (java.util.Iterator<com.google.javascript.rhino.Node> iter = exceptionHandler.iterator(); iter.hasNext();) {
			...
			if (com.google.javascript.jscomp.NodeUtil.isFunction(curHandler)) {
				break;
			}
	...
	private static com.google.javascript.rhino.Node computeFollowNode(com.google.javascript.rhino.Node fromNode, com.google.javascript.rhino.Node node, com.google.javascript.jscomp.ControlFlowAnalysis cfa) {
		com.google.javascript.rhino.Node parent = node.getParent();
		if (((parent == null) || ((parent.getType()) == (com.google.javascript.rhino.Token.FUNCTION))) || ((cfa != null) && (node == (cfa.root)))) {
			return null;
		}
		switch (parent.getType()) {
			case com.google.javascript.rhino.Token.IF :
			...
			case com.google.javascript.rhino.Token.DEFAULT :
				if ((parent.getNext()) != null) {
					if ((parent.getNext().getType()) == (com.google.javascript.rhino.Token.CASE)) {
						return parent.getNext().getFirstChild().getNext();
					}else
						if ((parent.getNext().getType()) == (com.google.javascript.rhino.Token.DEFAULT)) {
		...
		while ((nextSibling != null) && ((nextSibling.getType()) == (com.google.javascript.rhino.Token.FUNCTION))) {
	...
	private void connectToPossibleExceptionHandler(com.google.javascript.rhino.Node cfgNode, com.google.javascript.rhino.Node target) {
		if ((com.google.javascript.jscomp.ControlFlowAnalysis.mayThrowException(target)) && (!(exceptionHandler.isEmpty()))) {
			...
			for (com.google.javascript.rhino.Node handler : exceptionHandler) {
				if (com.google.javascript.jscomp.NodeUtil.isFunction(handler)) {
					return ;
				}
				com.google.common.base.Preconditions.checkState(((handler.getType()) == (com.google.javascript.rhino.Token.TRY)));
	...
	private static boolean matchLabel(com.google.javascript.rhino.Node target, java.lang.String label) {
		...
		while ((target.getType()) == (com.google.javascript.rhino.Token.LABEL)) {
	...
	static com.google.javascript.rhino.Node getExceptionHandler(com.google.javascript.rhino.Node n) {
		for (com.google.javascript.rhino.Node cur = n; ((cur.getType()) != (com.google.javascript.rhino.Token.SCRIPT)) && ((cur.getType()) != (com.google.javascript.rhino.Token.FUNCTION)); cur = cur.getParent()) {
	...
	static com.google.javascript.rhino.Node getCatchHandlerForBlock(com.google.javascript.rhino.Node block) {
		if ((((block.getType()) == (com.google.javascript.rhino.Token.BLOCK)) && ((block.getParent().getType()) == (com.google.javascript.rhino.Token.TRY))) && ((block.getParent().getFirstChild()) == block)) {
	...
	private static class AstControlFlowGraph extends com.google.javascript.jscomp.ControlFlowGraph<com.google.javascript.rhino.Node> {
		...
		private AstControlFlowGraph(com.google.javascript.rhino.Node entry, java.util.Map<com.google.javascript.jscomp.graph.DiGraph.DiGraphNode<com.google.javascript.rhino.Node, com.google.javascript.jscomp.ControlFlowGraph.Branch>, java.lang.Integer> priorities) {
			super(entry);
