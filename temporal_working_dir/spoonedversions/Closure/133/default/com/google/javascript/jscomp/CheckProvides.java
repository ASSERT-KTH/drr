	...
	private class CheckProvidesCallback extends com.google.javascript.jscomp.NodeTraversal.AbstractShallowCallback {
		...
		private void visitScriptNode() {
			for (java.util.Map.Entry<java.lang.String, com.google.javascript.rhino.Node> ctorEntry : ctors.entrySet()) {
				java.lang.String ctor = ctorEntry.getKey();
				int index = -1;
				boolean found = false;
				do {
					index = ctor.indexOf('.', (index + 1));
					java.lang.String provideKey = index == (-1) ? ctor : ctor.substring(0, index);
				...
				if (!found) {
					com.google.javascript.rhino.Node n = ctorEntry.getValue();
					compiler.report(com.google.javascript.jscomp.JSError.make(n.getSourceFileName(), n, checkLevel, com.google.javascript.jscomp.CheckProvides.MISSING_PROVIDE_WARNING, ctorEntry.getKey()));
				}
			}
			provides.clear();
			ctors.clear();
