...
class ProcessDefines implements com.google.javascript.jscomp.CompilerPass {
	...
	static final com.google.javascript.jscomp.DiagnosticType INVALID_DEFINE_TYPE_ERROR = com.google.javascript.jscomp.DiagnosticType.error("JSC_INVALID_DEFINE_TYPE_ERROR", "@define tag only permits literal types");
	...
	static final com.google.javascript.jscomp.DiagnosticType DEFINE_NOT_ASSIGNABLE_ERROR = com.google.javascript.jscomp.DiagnosticType.error("JSC_DEFINE_NOT_ASSIGNABLE_ERROR", "@define variable {0} cannot be reassigned due to code at {1}.");
	...
	private java.util.Map<java.lang.String, com.google.javascript.jscomp.ProcessDefines.DefineInfo> collectDefines(com.google.javascript.rhino.Node root, com.google.javascript.jscomp.GlobalNamespace namespace) {
		...
		for (com.google.javascript.jscomp.GlobalNamespace.Name name : namespace.getNameIndex().values()) {
			com.google.javascript.jscomp.GlobalNamespace.Ref decl = name.getDeclaration();
			if (((name.docInfo) != null) && (name.docInfo.isDefine())) {
				if (isValidDefineType(name.docInfo.getType())) {
					allDefines.add(name);
				}else {
					com.google.javascript.jscomp.JSError error = com.google.javascript.jscomp.JSError.make(decl.getSourceName(), decl.node, com.google.javascript.jscomp.ProcessDefines.INVALID_DEFINE_TYPE_ERROR);
	...
	private static final class CollectDefines implements com.google.javascript.jscomp.NodeTraversal.Callback {
		...
		CollectDefines(com.google.javascript.jscomp.AbstractCompiler compiler, java.util.List<com.google.javascript.jscomp.GlobalNamespace.Name> listOfDefines) {
			...
			for (com.google.javascript.jscomp.GlobalNamespace.Name name : listOfDefines) {
				com.google.javascript.jscomp.GlobalNamespace.Ref decl = name.getDeclaration();
				if (decl != null) {
					allRefInfo.put(decl.node, new com.google.javascript.jscomp.ProcessDefines.CollectDefines.RefInfo(decl, name));
				}
				for (com.google.javascript.jscomp.GlobalNamespace.Ref ref : name.getRefs()) {
					if (ref == decl) {
						continue;
			...
			if (refInfo != null) {
				...
				switch (ref.type) {
					...
					case SET_FROM_LOCAL :
						...
						if ((((valParent.getType()) == (com.google.javascript.rhino.Token.ASSIGN)) && (name.isSimpleName())) && ((name.getDeclaration()) == ref)) {
