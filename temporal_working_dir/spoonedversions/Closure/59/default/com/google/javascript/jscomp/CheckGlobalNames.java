...
class CheckGlobalNames implements com.google.javascript.jscomp.CompilerPass {
			...
			if ((!isDefined) && (!(isTypedef(ref)))) {
				...
				if (((declaration != null) && ((ref.getModule()) != (declaration.getModule()))) && (!(moduleGraph.dependsOn(ref.getModule(), declaration.getModule())))) {
					...
					if (((ref.scope.isGlobal()) && singleGlobalParentDecl) && ((parent.getDeclaration().preOrderIndex) > (ref.preOrderIndex))) {
						compiler.report(com.google.javascript.jscomp.JSError.make(ref.source.getName(), ref.node, com.google.javascript.jscomp.CheckGlobalNames.NAME_DEFINED_LATE_WARNING, name.fullName(), parent.fullName(), parent.getDeclaration().source.getName(), java.lang.String.valueOf(parent.getDeclaration().node.getLineno())));
					}
	...
	private void reportBadModuleReference(com.google.javascript.jscomp.GlobalNamespace.Name name, com.google.javascript.jscomp.GlobalNamespace.Ref ref) {
		compiler.report(com.google.javascript.jscomp.JSError.make(ref.source.getName(), ref.node, com.google.javascript.jscomp.CheckGlobalNames.STRICT_MODULE_DEP_QNAME, ref.getModule().getName(), name.getDeclaration().getModule().getName(), name.fullName()));
	}
	...
	private void reportRefToUndefinedName(com.google.javascript.jscomp.GlobalNamespace.Name name, com.google.javascript.jscomp.GlobalNamespace.Ref ref) {
		...
		compiler.report(com.google.javascript.jscomp.JSError.make(ref.getSourceName(), ref.node, level, com.google.javascript.jscomp.CheckGlobalNames.UNDEFINED_NAME_WARNING, name.fullName()));
	}
