...
class InlineSimpleMethods extends com.google.javascript.jscomp.MethodCompilerPass {
	private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(com.google.javascript.jscomp.InlineSimpleMethods.class.getName());

	InlineSimpleMethods(com.google.javascript.jscomp.AbstractCompiler compiler) {
		super(compiler);
	}
		...
		void visit(com.google.javascript.jscomp.NodeTraversal t, com.google.javascript.rhino.Node callNode, com.google.javascript.rhino.Node parent, java.lang.String callName) {
			...
			if (((definitions.size()) == 1) || (allDefinitionsEquivalent(definitions))) {
				if (!(argsMayHaveSideEffects(callNode))) {
					com.google.javascript.rhino.Node returned = com.google.javascript.jscomp.InlineSimpleMethods.returnedExpression(firstDefinition);
					if (returned != null) {
						if (com.google.javascript.jscomp.InlineSimpleMethods.isPropertyTree(returned)) {
							com.google.javascript.jscomp.InlineSimpleMethods.logger.fine(("Inlining property accessor: " + callName));
							inlinePropertyReturn(parent, callNode, returned);
						}else
							if ((com.google.javascript.jscomp.NodeUtil.isLiteralValue(returned, false)) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(callNode.getFirstChild(), compiler)))) {
								com.google.javascript.jscomp.InlineSimpleMethods.logger.fine(("Inlining constant accessor: " + callName));
								inlineConstReturn(parent, callNode, returned);
							}
						
					}else
						if ((com.google.javascript.jscomp.InlineSimpleMethods.isEmptyMethod(firstDefinition)) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(callNode.getFirstChild(), compiler)))) {
							com.google.javascript.jscomp.InlineSimpleMethods.logger.fine(("Inlining empty method: " + callName));
							inlineEmptyMethod(parent, callNode);
						}
					
				}
			}else {
				com.google.javascript.jscomp.InlineSimpleMethods.logger.fine((("Method '" + callName) + "' has conflicting definitions."));
			}
		}
	}
	...
	com.google.javascript.jscomp.NodeTraversal.Callback getActingCallback() {
		return new com.google.javascript.jscomp.InlineSimpleMethods.InlineTrivialAccessors();
	}
	...
	private static boolean isPropertyTree(com.google.javascript.rhino.Node expectedGetprop) {
		...
		if ((!(leftChild.isThis())) && (!(com.google.javascript.jscomp.InlineSimpleMethods.isPropertyTree(leftChild)))) {
			return false;
		}
	...
	private static void replaceThis(com.google.javascript.rhino.Node expectedGetprop, com.google.javascript.rhino.Node replacement) {
		...
		if (leftChild.isThis()) {
			...
			com.google.javascript.jscomp.InlineSimpleMethods.replaceThis(leftChild, replacement);
		}
	}

	private static com.google.javascript.rhino.Node returnedExpression(com.google.javascript.rhino.Node fn) {
		com.google.javascript.rhino.Node expectedBlock = com.google.javascript.jscomp.InlineSimpleMethods.getMethodBlock(fn);
	...
	private void inlinePropertyReturn(com.google.javascript.rhino.Node parent, com.google.javascript.rhino.Node call, com.google.javascript.rhino.Node returnedValue) {
		...
		com.google.javascript.jscomp.InlineSimpleMethods.replaceThis(getProp, call.getFirstChild().removeFirstChild());
	...
	private void inlineEmptyMethod(com.google.javascript.rhino.Node parent, com.google.javascript.rhino.Node call) {
		if (com.google.javascript.jscomp.NodeUtil.isExprCall(parent)) {
			parent.getParent().replaceChild(parent, com.google.javascript.rhino.IR.empty());
		}else {
	...
	private boolean argsMayHaveSideEffects(com.google.javascript.rhino.Node call) {
		for (com.google.javascript.rhino.Node currentChild = call.getFirstChild().getNext(); currentChild != null; currentChild = currentChild.getNext()) {
			if (com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(currentChild, compiler)) {
				return true;
			}
		}
		return false;
	...
	com.google.javascript.jscomp.MethodCompilerPass.SignatureStore getSignatureStore() {
		return com.google.javascript.jscomp.InlineSimpleMethods.DUMMY_SIGNATURE_STORE;
	}
}
