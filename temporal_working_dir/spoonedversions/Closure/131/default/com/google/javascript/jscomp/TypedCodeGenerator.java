...
class TypedCodeGenerator extends com.google.javascript.jscomp.CodeGenerator {
	...
	TypedCodeGenerator(com.google.javascript.jscomp.CodeConsumer consumer, com.google.javascript.jscomp.CompilerOptions options, com.google.javascript.rhino.jstype.JSTypeRegistry registry) {
		super(consumer, options);
		com.google.common.base.Preconditions.checkNotNull(registry);
	...
	private java.lang.String getFunctionAnnotation(com.google.javascript.rhino.Node fnNode) {
		...
		if (com.google.javascript.rhino.jstype.JSType.isEquivalent(type, registry.getNativeType(com.google.javascript.rhino.jstype.JSTypeNative.FUNCTION_INSTANCE_TYPE))) {
		...
		if (((retType != null) && (!(retType.isEmptyType()))) && (!(funType.isInterface()))) {
			sb.append(" * ");
		...
		if (!(funType.getTemplateTypeMap().getTemplateKeys().isEmpty())) {
			sb.append(" * @template ");
			sb.append(com.google.common.base.Joiner.on(",").join(funType.getTemplateTypeMap().getTemplateKeys()));
			sb.append("\n");
		}
	...
	private com.google.javascript.rhino.jstype.JSType restrictByUndefined(com.google.javascript.rhino.jstype.JSType type) {
		if (type.isUnionType()) {
			return type.toMaybeUnionType().getRestrictedUnion(registry.getNativeType(com.google.javascript.rhino.jstype.JSTypeNative.VOID_TYPE));
		}
		return type;
	}
}
