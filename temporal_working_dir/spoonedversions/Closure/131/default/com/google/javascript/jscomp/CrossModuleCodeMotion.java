...
class CrossModuleCodeMotion extends com.google.javascript.jscomp.NodeTraversal.AbstractPostOrderCallback implements com.google.javascript.jscomp.CompilerPass {
	...
	public void process(com.google.javascript.rhino.Node externs, com.google.javascript.rhino.Node root) {
		com.google.javascript.jscomp.CrossModuleCodeMotion.logger.fine("Moving functions + variable into deeper modules");
			...
			if ((info.allowMove) && (deepestDependency != null)) {
				...
				while (it.hasNext()) {
					...
					if (((decl.module) != null) && (moduleGraph.dependsOn(deepestDependency, decl.module))) {
						...
						com.google.common.base.Preconditions.checkState(((!(declParent.isVar())) || (declParent.hasOneChild())), "AST not normalized.");
	...
	private class Declaration {
		...
		Declaration(com.google.javascript.jscomp.JSModule module, com.google.javascript.rhino.Node node) {
			this.module = module;
			this.node = node;
		}
	}
		...
		switch (parent.getType()) {
			case com.google.javascript.rhino.Token.VAR :
				if (canMoveValue(name.getFirstChild())) {
					return info.addDeclaration(new com.google.javascript.jscomp.CrossModuleCodeMotion.Declaration(t.getModule(), name));
				}
				return false;
			case com.google.javascript.rhino.Token.FUNCTION :
					...
					if (current.isGetProp()) {
						...
						if ((current.isAssign()) && ((current.getFirstChild()) == child)) {
							...
							if ((currentParent.isExprResult()) && (canMoveValue(current.getLastChild()))) {
								return info.addDeclaration(new com.google.javascript.jscomp.CrossModuleCodeMotion.Declaration(t.getModule(), current));
							}
						}else {
							return false;
						}
			...
			case com.google.javascript.rhino.Token.CALL :
				if (com.google.javascript.jscomp.NodeUtil.isExprCall(gramps)) {
					...
					if ((relationship != null) && (name.getString().equals(relationship.subclassName))) {
						return info.addDeclaration(new com.google.javascript.jscomp.CrossModuleCodeMotion.Declaration(t.getModule(), parent));
					}
				}
				return false;
			default :
