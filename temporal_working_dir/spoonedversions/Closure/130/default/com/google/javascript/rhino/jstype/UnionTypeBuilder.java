...
class UnionTypeBuilder implements java.io.Serializable {
	...
	java.util.Collection<com.google.javascript.rhino.jstype.JSType> getAlternates() {
					...
					if ((((((alternate.isUnknownType()) || (current.isUnknownType())) || (alternate.isNoResolvedType())) || (current.isNoResolvedType())) || (alternate.hasAnyTemplateTypes())) || (current.hasAnyTemplateTypes())) {
						...
						if ((alternate.isTemplatizedType()) || (current.isTemplatizedType())) {
							if (!(current.isTemplatizedType())) {
								...
								if (!(alternate.isTemplatizedType())) {
									if (current.isSubtype(alternate)) {
										removeCurrent = true;
									}
								}else {
									com.google.common.base.Preconditions.checkState(((current.isTemplatizedType()) && (alternate.isTemplatizedType())));
									com.google.javascript.rhino.jstype.TemplatizedType templatizedAlternate = alternate.toMaybeTemplatizedType();
									com.google.javascript.rhino.jstype.TemplatizedType templatizedCurrent = current.toMaybeTemplatizedType();
									if (templatizedCurrent.wrapsSameRawType(templatizedAlternate)) {
										if (alternate.getTemplateTypeMap().checkEquivalenceHelper(current.getTemplateTypeMap(), com.google.javascript.rhino.jstype.EquivalenceMethod.IDENTITY)) {
											return com.google.javascript.rhino.jstype.UnionTypeBuilder.this;
										}else {
											com.google.javascript.rhino.jstype.JSType merged = templatizedCurrent.getReferencedObjTypeInternal();
