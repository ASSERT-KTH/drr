		...
		if (node != null) {
			...
			for (com.google.javascript.rhino.Node current = node; current != null; current = current.getParent()) {
				...
				if (type == (com.google.javascript.rhino.Token.FUNCTION)) {
					info = com.google.javascript.jscomp.NodeUtil.getBestJSDocInfo(current);
					visitedFunction = true;
				}else
					if (type == (com.google.javascript.rhino.Token.SCRIPT)) {
						info = current.getJSDocInfo();
					}else
						if ((current.isVar()) || (current.isAssign())) {
							com.google.javascript.rhino.Node rhs = com.google.javascript.jscomp.NodeUtil.getRValueOfLValue(current.getFirstChild());
							if (rhs != null) {
								if (rhs.isCast()) {
