...
class InferJSDocInfo extends com.google.javascript.jscomp.NodeTraversal.AbstractPostOrderCallback implements com.google.javascript.jscomp.CompilerPass {
		...
		if (((objType.isConstructor()) || (objType.isEnumType())) || (objType.isInterface())) {
			if ((objType.hasReferenceName()) && (objType.getReferenceName().equals(qName))) {
				...
				if ((objType.isConstructor()) || (objType.isInterface())) {
					((com.google.javascript.rhino.jstype.FunctionType) (objType)).getInstanceType().setJSDocInfo(docInfo);
				}else
