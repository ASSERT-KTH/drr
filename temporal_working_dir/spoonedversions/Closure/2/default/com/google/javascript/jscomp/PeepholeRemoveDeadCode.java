...
class PeepholeRemoveDeadCode extends com.google.javascript.jscomp.AbstractPeepholeOptimization {
	...
	private com.google.javascript.rhino.Node tryOptimizeSwitch(com.google.javascript.rhino.Node n) {
		...
		if (defaultCase == null) {
			...
			if (com.google.javascript.jscomp.NodeUtil.isLiteralValue(cond, false)) {
				com.google.javascript.rhino.Node caseLabel;
				com.google.javascript.rhino.jstype.TernaryValue caseMatches = com.google.javascript.rhino.jstype.TernaryValue.TRUE;
				for (cur = cond.getNext(); cur != null; cur = next) {
					next = cur.getNext();
					caseLabel = cur.getFirstChild();
					caseMatches = com.google.javascript.jscomp.PeepholeFoldConstants.evaluateComparison(com.google.javascript.rhino.Token.SHEQ, cond, caseLabel);
					if (caseMatches == (com.google.javascript.rhino.jstype.TernaryValue.TRUE)) {
						break;
					}else
						if (caseMatches == (com.google.javascript.rhino.jstype.TernaryValue.UNKNOWN)) {
							break;
						}else {
							removeCase(n, cur);
						}
				...
				if (caseMatches != (com.google.javascript.rhino.jstype.TernaryValue.UNKNOWN)) {
					...
					if ((cur != null) && ((cur.getNext()) == null)) {
						block = cur.getLastChild();
						if (!(com.google.javascript.jscomp.NodeUtil.containsType(block, com.google.javascript.rhino.Token.BREAK, com.google.javascript.jscomp.NodeUtil.MATCH_NOT_FUNCTION))) {
