...
class RemoveUnusedVars implements com.google.javascript.jscomp.CompilerPass , com.google.javascript.jscomp.OptimizeCalls.CallGraphCompilerPass {
	...
	private final com.google.common.collect.Multimap<com.google.javascript.jscomp.Scope.Var, com.google.javascript.rhino.Node> classDefiningCalls = com.google.common.collect.ArrayListMultimap.create();
	...
	private void traverseNode(com.google.javascript.rhino.Node n, com.google.javascript.rhino.Node parent, com.google.javascript.jscomp.Scope scope) {
		...
		switch (type) {
			...
			case com.google.javascript.rhino.Token.CALL :
				com.google.javascript.jscomp.Scope.Var modifiedVar = null;
				...
				if (subclassRelationship != null) {
					...
					java.lang.String className = codingConvention.getSingletonGetterClassName(n);
					if (className != null) {
				...
				if (((modifiedVar != null) && (modifiedVar.isGlobal())) && (!(referenced.contains(modifiedVar)))) {
					...
					continuations.put(modifiedVar, new com.google.javascript.jscomp.RemoveUnusedVars.Continuation(n, scope));
					return ;
				}
				break;
			case com.google.javascript.rhino.Token.NAME :
				...
				if (parent.isVar()) {
					...
					if ((((value != null) && (var != null)) && (isRemovableVar(var))) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(value, compiler)))) {
						continuations.put(var, new com.google.javascript.jscomp.RemoveUnusedVars.Continuation(n, scope));
						return ;
					}
				}else {
	...
	private static class CallSiteOptimizer {
		...
		public void applyChanges() {
			...
			for (com.google.javascript.rhino.Node n : toReplaceWithZero) {
				n.getParent().replaceChild(n, com.google.javascript.rhino.IR.number(0).srcref(n));
				compiler.reportCodeChange();
			}
		}
				...
				if (com.google.javascript.jscomp.RemoveUnusedVars.CallSiteOptimizer.isModifiableCallSite(site)) {
					...
					if (arg != null) {
						if (canModifyAllSites || (((arg.getNext()) == null) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(arg, compiler))))) {
							...
							if ((!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(arg, compiler))) && ((!(arg.isNumber())) || ((arg.getDouble()) != 0))) {
		...
		private boolean canChangeSignature(com.google.javascript.rhino.Node function) {
			...
			for (com.google.javascript.jscomp.UseSite site : useSites) {
				...
				if (!(com.google.javascript.jscomp.SimpleDefinitionFinder.isCallOrNewSite(site))) {
					if (!((parent.isGetProp()) && (com.google.javascript.jscomp.NodeUtil.isFunctionObjectCall(parent.getParent())))) {
						return false;
					}
				}
		...
		do {
			...
			for (int current = 0; current < (maybeUnreferenced.size()); current++) {
				...
				if (referenced.contains(var)) {
					...
					if ((var.getParentNode().isVar()) && (!(com.google.javascript.jscomp.NodeUtil.isForIn(var.getParentNode().getParent())))) {
	...
	private void removeUnreferencedVars() {
		for (java.util.Iterator<com.google.javascript.jscomp.Scope.Var> it = maybeUnreferenced.iterator(); it.hasNext();) {
			com.google.javascript.jscomp.Scope.Var var = it.next();
			for (com.google.javascript.rhino.Node exprCallNode : classDefiningCalls.get(var)) {
			...
			com.google.common.base.Preconditions.checkState((((toRemove.isVar()) || (toRemove.isFunction())) || ((toRemove.isParamList()) && (parent.isFunction()))), "We should only declare vars and functions and function args");
			if ((toRemove.isParamList()) && (parent.isFunction())) {
				...
				if (com.google.javascript.jscomp.NodeUtil.isFunctionExpression(toRemove)) {
					...
					if (((parent != null) && (parent.isFor())) && ((parent.getChildCount()) < 4)) {
						...
						if (((toRemove.isVar()) && (nameNode.hasChildren())) && (com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(nameNode.getFirstChild(), compiler))) {
							if ((toRemove.getChildCount()) == 1) {
								parent.replaceChild(toRemove, com.google.javascript.rhino.IR.exprResult(nameNode.removeFirstChild()));
								compiler.reportCodeChange();
							}
						}else
	...
	private static class Assign {
		...
		Assign(com.google.javascript.rhino.Node assignNode, com.google.javascript.rhino.Node nameNode, boolean isPropertyAssign) {
			...
			this.maybeAliased = com.google.javascript.jscomp.NodeUtil.isExpressionResultUsed(assignNode);
			this.mayHaveSecondarySideEffects = ((maybeAliased) || (com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(assignNode.getFirstChild()))) || (com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(assignNode.getLastChild()));
		}
			...
			if (mayHaveSecondarySideEffects) {
				...
				for (com.google.javascript.rhino.Node current = assignNode.getFirstChild(); !(current.isName()); current = current.getFirstChild()) {
					if (current.isGetElem()) {
						replacement = com.google.javascript.rhino.IR.comma(current.getLastChild().detachFromParent(), replacement);
