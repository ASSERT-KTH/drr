...
class DisambiguateProperties<T> implements com.google.javascript.jscomp.CompilerPass {
	...
	static class Warnings {
		...
		static final com.google.javascript.jscomp.DiagnosticType INVALIDATION_ON_TYPE = com.google.javascript.jscomp.DiagnosticType.disabled("JSC_INVALIDATION_TYPE", ("Property disambiguator skipping instances of property {0} " + "on type {1}. {2}"));
	}
	...
	void renameProperties() {
		...
		java.util.Set<java.lang.String> reported = com.google.common.collect.Sets.newHashSet();
			...
			if (prop.shouldRename()) {
				...
				for (com.google.javascript.rhino.Node node : prop.renameNodes) {
					...
					if (prop.shouldRename(rootType)) {
						...
						++instancesSkipped;
						com.google.javascript.jscomp.CheckLevel checkLevelForProp = propertiesToErrorFor.get(prop.name);
						if (((checkLevelForProp != null) && (checkLevelForProp != (com.google.javascript.jscomp.CheckLevel.OFF))) && (!(reported.contains(prop.name)))) {
							reported.add(prop.name);
							compiler.report(com.google.javascript.jscomp.JSError.make(com.google.javascript.jscomp.NodeUtil.getSourceName(node), node, checkLevelForProp, com.google.javascript.jscomp.DisambiguateProperties.Warnings.INVALIDATION_ON_TYPE, prop.name, rootType.toString(), ""));
						}
					}
				}
			}else {
