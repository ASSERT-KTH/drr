		...
		private void checkVar(com.google.javascript.jscomp.NodeTraversal t, com.google.javascript.jscomp.Scope.Var v, java.util.List<com.google.javascript.jscomp.ReferenceCollectingCallback.Reference> references) {
			...
			for (com.google.javascript.jscomp.ReferenceCollectingCallback.Reference reference : references) {
				...
				if (isDeclaration) {
					for (com.google.javascript.jscomp.ReferenceCollectingCallback.BasicBlock declaredBlock : blocksWithDeclarations) {
						if (declaredBlock.provablyExecutesBefore(basicBlock)) {
							compiler.report(com.google.javascript.jscomp.JSError.make(reference.getSourceFile().getName(), reference.getNode(), checkLevel, com.google.javascript.jscomp.VariableReferenceCheck.REDECLARED_VARIABLE, v.name));
							break;
						}
					}
				}
				if ((isUnhoistedNamedFunction && (!isDeclaration)) && isDeclaredInScope) {
					for (com.google.javascript.jscomp.ReferenceCollectingCallback.BasicBlock declaredBlock : blocksWithDeclarations) {
						if (!(declaredBlock.provablyExecutesBefore(basicBlock))) {
							compiler.report(com.google.javascript.jscomp.JSError.make(reference.getSourceFile().getName(), reference.getNode(), com.google.javascript.jscomp.VariableReferenceCheck.AMBIGUOUS_FUNCTION_DECL, v.name));
							break;
						}
					}
				}
				if ((!isDeclaration) && (!isDeclaredInScope)) {
					...
					if ((reference.getScope()) == (v.scope)) {
						compiler.report(com.google.javascript.jscomp.JSError.make(reference.getSourceFile().getName(), reference.getNode(), checkLevel, com.google.javascript.jscomp.VariableReferenceCheck.UNDECLARED_REFERENCE, v.name));
					}
				}
				if (isDeclaration) {
