...
class RemoveUnusedVars implements com.google.javascript.jscomp.CompilerPass , com.google.javascript.jscomp.OptimizeCalls.CallGraphCompilerPass {
	...
	private static class CallSiteOptimizer {
		...
		public void optimize(com.google.javascript.jscomp.Scope fnScope, java.util.Set<com.google.javascript.jscomp.Scope.Var> referenced) {
			...
			removeUnreferencedFunctionArgs(fnScope, function, referenced, argList.getFirstChild(), 0, changeCallSignature);
		}

		private boolean removeUnreferencedFunctionArgs(com.google.javascript.jscomp.Scope scope, com.google.javascript.rhino.Node function, java.util.Set<com.google.javascript.jscomp.Scope.Var> referenced, com.google.javascript.rhino.Node param, int paramIndex, boolean canChangeSignature) {
			if (param != null) {
				...
				if (!(referenced.contains(var))) {
					...
					if (modifyAllCallSites || (!hasFollowing)) {
						com.google.javascript.jscomp.RemoveUnusedVars.getFunctionArgList(function).removeChild(param);
						compiler.reportCodeChange();
						return hasFollowing;
					}
				}
		...
		private boolean canRemoveArgFromCallSites(com.google.javascript.rhino.Node function, int argIndex) {
			...
			for (com.google.javascript.jscomp.UseSite site : defFinder.getUseSites(definition)) {
				if (com.google.javascript.jscomp.RemoveUnusedVars.CallSiteOptimizer.isModifableCallSite(site)) {
					com.google.javascript.rhino.Node arg = com.google.javascript.jscomp.NodeUtil.getArgumentForCallOrNew(site.node.getParent(), argIndex);
					...
					if (arg != null) {
						...
						if (canModifyAllSites || (((arg.getNext()) == null) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(arg, compiler))))) {
							...
							if ((!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(arg, compiler))) && (((arg.getType()) != (com.google.javascript.rhino.Token.NUMBER)) || ((arg.getDouble()) != 0))) {
								argParent.replaceChild(arg, com.google.javascript.rhino.Node.newNumber(0).copyInformationFrom(arg));
								compiler.reportCodeChange();
							}
						}
					}
		...
		private void tryRemoveAllFollowingArgs(com.google.javascript.rhino.Node function, final int argIndex) {
			...
			for (com.google.javascript.jscomp.UseSite site : defFinder.getUseSites(definition)) {
				...
				com.google.javascript.rhino.Node arg = com.google.javascript.jscomp.NodeUtil.getArgumentForCallOrNew(site.node.getParent(), argIndex);
				while (arg != null) {
					com.google.javascript.rhino.Node next = arg.getNext();
					if ((next != null) && (!(com.google.javascript.jscomp.NodeUtil.mayHaveSideEffects(next)))) {
		...
		private static boolean isModifableCallSite(com.google.javascript.jscomp.UseSite site) {
			return (com.google.javascript.jscomp.SimpleDefinitionFinder.isCallOrNewSite(site)) && (!(com.google.javascript.jscomp.NodeUtil.isFunctionObjectCallOrApply(site.node.getParent())));
		}
		...
		private boolean canChangeSignature(com.google.javascript.rhino.Node function) {
			...
			for (com.google.javascript.jscomp.UseSite site : useSites) {
				...
				if (com.google.javascript.jscomp.NodeUtil.isFunctionObjectCallOrApply(site.node.getParent())) {
					return false;
				}
				com.google.javascript.rhino.Node nameNode = site.node;
				java.util.Collection<com.google.javascript.jscomp.DefinitionsRemover.Definition> singleSiteDefinitions = defFinder.getDefinitionsReferencedAt(nameNode);
