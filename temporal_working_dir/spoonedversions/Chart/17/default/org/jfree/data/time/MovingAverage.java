...
public class MovingAverage {
	...
	public static org.jfree.data.time.TimeSeries createMovingAverage(org.jfree.data.time.TimeSeries source, java.lang.String name, int periodCount, int skip) {
		...
		org.jfree.data.time.TimeSeries result = new org.jfree.data.time.TimeSeries(name, source.getTimePeriodClass());
		if ((source.getItemCount()) > 0) {
			long firstSerial = (source.getDataItem(0).getPeriod().getSerialIndex()) + skip;
			for (int i = (source.getItemCount()) - 1; i >= 0; i--) {
				org.jfree.data.time.TimeSeriesDataItem current = source.getDataItem(i);
				...
				if (serial >= firstSerial) {
					...
					while ((offset < periodCount) && (!finished)) {
						if ((i - offset) >= 0) {
							org.jfree.data.time.TimeSeriesDataItem item = source.getDataItem((i - offset));
	...
	public static org.jfree.data.time.TimeSeries createPointMovingAverage(org.jfree.data.time.TimeSeries source, java.lang.String name, int pointCount) {
		...
		for (int i = 0; i < (source.getItemCount()); i++) {
			...
			if (i > (pointCount - 1)) {
				org.jfree.data.time.TimeSeriesDataItem startOfMovingAvg = source.getDataItem((i - pointCount));
