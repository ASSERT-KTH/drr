

package org.jfree.chart.urls;


public class CustomXYURLGenerator implements java.io.Serializable , org.jfree.chart.urls.XYURLGenerator {
	private static final long serialVersionUID = -8565933356596551832L;

	private java.util.ArrayList urlSeries = new java.util.ArrayList();

	public CustomXYURLGenerator() {
		super();
	}

	public int getListCount() {
		return org.jfree.chart.urls.CustomXYURLGenerator.this.urlSeries.size();
	}

	public int getURLCount(int list) {
		int result = 0;
		java.util.List urls = ((java.util.List) (org.jfree.chart.urls.CustomXYURLGenerator.this.urlSeries.get(list)));
		if (urls != null) {
			result = urls.size();
		}
		return result;
	}

	public java.lang.String getURL(int series, int item) {
		java.lang.String result = null;
		if (series < (getListCount())) {
			java.util.List urls = ((java.util.List) (org.jfree.chart.urls.CustomXYURLGenerator.this.urlSeries.get(series)));
			if (urls != null) {
				if (item < (urls.size())) {
					result = ((java.lang.String) (urls.get(item)));
				}
			}
		}
		return result;
	}

	public java.lang.String generateURL(org.jfree.data.xy.XYDataset dataset, int series, int item) {
		return getURL(series, item);
	}

	public void addURLSeries(java.util.List urls) {
		org.jfree.chart.urls.CustomXYURLGenerator.this.urlSeries.add(urls);
	}

	public boolean equals(java.lang.Object o) {
		if (o == null) {
			return false;
		}
		if (o == (org.jfree.chart.urls.CustomXYURLGenerator.this)) {
			return true;
		}
		if (!(o instanceof org.jfree.chart.urls.CustomXYURLGenerator)) {
			return false;
		}
		org.jfree.chart.urls.CustomXYURLGenerator generator = ((org.jfree.chart.urls.CustomXYURLGenerator) (o));
		int listCount = getListCount();
		if (listCount != (generator.getListCount())) {
			return false;
		}
		for (int series = 0; series < listCount; series++) {
			int urlCount = getURLCount(series);
			if (urlCount != (generator.getURLCount(series))) {
				return false;
			}
			for (int item = 0; item < urlCount; item++) {
				java.lang.String u1 = getURL(series, item);
				java.lang.String u2 = generator.getURL(series, item);
				if (u1 != null) {
					if (!(u1.equals(u2))) {
						return false;
					}
				}else {
					if (u2 != null) {
						return false;
					}
				}
			}
		}
		return true;
	}
}

